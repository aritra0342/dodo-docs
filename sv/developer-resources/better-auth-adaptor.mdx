...
<Warning>
Kom aldrig ihåg API-nycklar eller hemligheter i versionskontroll.
</Warning>
</Step>

<Step title="Ställ in server-sidan integration">
...
<Tip>
Ställ in <code>environment</code> till <code>live_mode</code> för produktion.
</Tip>
</Step>

<Step title="Ställ in klient-sidan integration">
...
<Info>
  ...
  integrationer. Den äldre <code>checkout</code>-metoden är avvecklad och hålls
...
<Info>
  ...
  ingen faktureringsinformation i förväg eftersom den kommer att tas från användaren
  på kassan. Du kan fortfarande åsidosätta det genom att skicka nyckeln
  <code>billing</code> i argumentet.
</Info>

<Info>
  Likt den äldre <code>checkout</code>-metoden tar <code>checkoutSession</code>
  kundens e-post och namn från deras
  better-auth-session, men du kan åsidosätta det genom att skicka <code>customer</code>
  objektet med e-post och namn.
</Info>

<Info>
  Du kan skicka samma alternativ i argumentet som begärningskropp för [Skapa
  Checkout
...
<Note>
  Retur-URL:en tas från <code>successUrl</code> som är konfigurerad i
  serverplugin. Du behöver inte inkludera <code>return_url</code> i
  klientpayloaden.
</Note>
...
<Warning>
  Metoden <code>authClient.dodopayments.checkout</code> är avvecklad. Använd
...
<Tip>
  Tidsstämplar som är äldre än en timme eller mer än fem minuter in i framtiden avvisas
  vid insamling av användning.
</Tip>
<Tip>
  Om du utelämnar <code>meter_id</code> när du listar användningsmätare, returneras alla mätare
  kopplade till kundens prenumerationer.
</Tip>

# Webhooks

<Info>
  Webhook-pluginet bearbetar realtidsbetalningsevenemang från Dodo Payments med
  säker signaturverifiering. Standardändpunkten är <code>/api/auth/dodopayments/webhooks</code>.
</Info>

<Steps>
<Step title="Generera och ställ in webhook-hemlighet">
Generera en webhook-hemlighet för din ändpunkt-URL (t.ex. <code>https://&lt;your-domain&gt;/api/auth/dodopayments/webhooks</code>) i Dodo Payments Dashboard och ställ in den i din <code>.env</code>-fil:
...
<AccordionGroup>
  <Accordion title="Pluginalternativ">
    - <b>client</b> (obligatorisk): DodoPayments klientinstans - <b>createCustomerOnSignUp</b> (valfritt): Automatisk skapa kunder vid användarregistrering - <b>use</b> (obligatorisk): Array av plugins att aktivera (checkout, portal, usage, webhooks)
  </Accordion>
  <Accordion title="Checkout Pluginalternativ">
    - <b>products</b>: Array av produkter eller asynkron funktion som returnerar produkter - <b>successUrl</b>: URL att omdirigera efter lyckad betalning - <b>authenticatedUsersOnly</b>: Kräver användarautentisering (standard: false)
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Vanliga problem">
    - <b>Ogiltig API-nyckel</b>: Dubbelkolla din <code>DODO_PAYMENTS_API_KEY</code> i <code>.env</code>. - <b>Webhook-signatur mismatch</b>: Se till att webhook-hemligheten matchar den som är inställd i Dodo Payments Dashboard. - <b>Kund skapades inte</b>: Bekräfta att <code>createCustomerOnSignUp</code> är inställt på <code>true</code>.
  </Accordion>
  <Accordion title="Bästa praxis">
    - Använd miljövariabler för alla hemligheter och nycklar. - Testa i <code>test_mode</code> innan du växlar till <code>live_mode</code>. - Logga webhook-händelser för felsökning och granskning.
  </Accordion>
</AccordionGroup>
