...
|--------|------|----------|-------------|
| `checkoutUrl` | `string` | Ja | URL för kassan. |
| `elementId` | `string` | Ja | `id` av DOM-elementet där kassan ska renderas. |
| `options.showTimer` | `boolean` | Nej | Visa eller dölja kassatimer. Standardinställningen är `true`. När den är inaktiverad får du `checkout.link_expired`-händelsen när sessionen går ut. |
...
2. **Synkronisering**: Använd `checkout.breakdown`-händelsen för att hålla din anpassade orderöversikt eller prissättningstabeller synkroniserade med vad användaren ser i kassa-ramen.
...
<AccordionGroup>
<Accordion title="Kassa-ramen visas inte">
...
Apple Pay gör det möjligt för kunder att slutföra betalningar snabbt och säkert med sina sparade betalningsmetoder. När det är aktiverat kan kunder starta Apple Pay-modalen direkt från kassa-överlagret på stödda enheter.
...
| Bäst för | Anpassade kassa-sidor, högkonverterande flöden | Snabb integration, befintliga sidor |

<Tip>
Använd **inline kassa** när du vill ha maximal kontroll över kassa-upplevelsen och sömlös branding. Använd **överkurs kassa** för snabbare integration med minimala ändringar i dina befintliga sidor.
</Tip>
...
<Card title="Kassa Sessions API" icon="code" href="/api-reference/checkout-sessions/create">
