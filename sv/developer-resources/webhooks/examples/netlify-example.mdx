...
> **Obs:** Vi kommer att ställa in `DODO_PAYMENTS_WEBHOOK_KEY` efter distribution när du har din webhook-URL.
...
> **Obs:** Denna implementation demonstrerar hantering av tre kärnprenumerationsevenemang (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) med minimala fält. Du kan enkelt utöka den för att stödja ytterligare händelsetyper och fält baserat på dina krav.
...
2. **Kontrollerar dubbletter** - Använder webhook-ID för att förhindra att samma händelse behandlas två gånger
3. **Loggar händelsen** - Lagrar rå webhook i `webhook_events` tabell för revisionsspår
4. **Behandlar händelsen** - Skapar eller uppdaterar kunder och prenumerationer i Neon
5. **Hanterar fel** - Loggar misslyckanden och markerar händelsen som obehandlad för återföring
...
| Funktion inte hittad (404) | Kör `netlify init` och `npm run deploy` igen |
