...
export default function CheckoutPage() {
  ...
  const [finalTotalCurrency, setFinalTotalCurrency] = useState<string | null>(null);
      ...
      onEvent: (event) => {
        ...
        if (event.event_type === "checkout.breakdown") {
          ...
          if (message) {
            ...
            setDiscount(message?.discount ?? null);
            setTotal(message?.total ?? null);
            setTax(message?.tax ?? null);
            setCurrency(message?.currency ?? null);
            setFinalTotal(message.finalTotal ?? null);
            setFinalTotalCurrency(message.finalTotalCurrency ?? null);
          }
        }
      }
    });
  ...
  const formattedAmt = (amt: number | null, currency: string | null) => {
    if (amt == null || currency == null) {
      return '0.00';
    }
    return `${currency} ${amt?.toFixed(2) ?? '0.00'}`;
  }
        ...
        <div className="space-y-2">
          {subTotal && (
            <div className="flex justify-between">
              <span>Subtotal</span>
              <span>{formattedAmt(subTotal, currency ?? '')}</span>
            </div>
          )}
          {discount && (
            <div className="flex justify-between">
              <span>Discount</span>
              <span>{formattedAmt(discount, currency ?? '')}</span>
            </div>
          )}
          {tax != null && (
            <div className="flex justify-between">
              <span>Tax</span>
              <span>{formattedAmt(tax, currency ?? '')}</span>
            </div>
          )}
          <hr />
          {(total || finalTotal) && (
            <div className="flex justify-between font-bold text-xl">
              <span>Total</span>
              <span>{formattedAmt(finalTotal ?? total ?? 0, finalTotalCurrency ?? currency ?? '')}</span>
            </div>
          )}
        </div>
      </div>
