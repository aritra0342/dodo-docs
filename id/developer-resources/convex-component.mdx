...
<Warning>
Sebelum menggunakan query ini, pastikan untuk mendefinisikan skema yang sesuai di file `convex/schema.ts` Anda atau ubah query agar sesuai dengan skema yang ada.
</Warning>
...
<Tab title="Pengaturan Penangan Webhook">
  <Info>
  Gunakan penangan ini untuk menerima dan memproses acara webhook Dodo Payments dengan aman di aplikasi Convex Anda. Semua penangan webhook menerima `ActionCtx` Convex sebagai parameter pertama, memungkinkan Anda menggunakan `ctx.runQuery()` dan `ctx.runMutation()` untuk berinteraksi dengan database Anda.
  </Info>
  <CodeGroup>
