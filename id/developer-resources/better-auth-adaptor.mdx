...
<Check>
Semua paket yang diperlukan sekarang telah terinstal.
</Check>
</Step>
</Steps>
...
<Warning>
Jangan pernah mengkomit kunci API atau rahasia ke kontrol versi.
</Warning>
</Step>

<Step title="Siapkan integrasi sisi server">
...
<Step title="Siapkan integrasi sisi klien">
...
<Info>
  Lebih baik menggunakan <code>authClient.dodopayments.checkoutSession</code> untuk integrasi baru.
  Metode <code>checkout</code> yang lama sudah tidak digunakan dan hanya dipertahankan
...
<Info>
  Berbeda dengan metode <code>checkout</code> yang lama, <code>checkoutSession</code>
  tidak memerlukan informasi penagihan di muka karena akan diambil dari pengguna
  di halaman checkout. Anda masih dapat menimpanya dengan mengirimkan
  kunci <code>billing</code> dalam argumen.
</Info>

<Info>
  Mirip dengan metode <code>checkout</code> yang lama,
  <code>checkoutSession</code> mengambil email dan nama pelanggan dari sesi
  better-auth mereka, namun, Anda dapat menimpanya dengan mengirimkan
  objek <code>customer</code> dengan email dan nama.
</Info>

<Info>
  Anda dapat mengirimkan opsi yang sama dalam argumen sebagai body permintaan untuk [Buat
  Sesi
...
<Note>
  URL pengembalian diambil dari <code>successUrl</code> yang dikonfigurasi di
  plugin server. Anda tidak perlu menyertakan <code>return_url</code> dalam
  payload klien.
</Note>
...
<Warning>
  Metode <code>authClient.dodopayments.checkout</code> sudah tidak digunakan. Gunakan
  <code>checkoutSession</code> sebagai gantinya untuk implementasi baru.
</Warning>
...
<Tip>
  Timestamps yang lebih tua dari satu jam atau lebih dari lima menit ke depan akan
  ditolak saat mengumpulkan penggunaan.
</Tip>
<Tip>
  Jika Anda mengabaikan <code>meter_id</code> saat mendaftar meter penggunaan, semua meter 
  yang terikat pada langganan pelanggan akan dikembalikan.
</Tip>

# Webhook

<Info>
  Plugin webhook memproses peristiwa pembayaran waktu nyata dari Dodo Payments dengan
  verifikasi tanda tangan yang aman. Endpoint default adalah <code>/api/auth/dodopayments/webhooks</code>.
</Info>

<Steps>
<Step title="Hasilkan dan atur rahasia webhook">
Hasilkan rahasia webhook untuk URL endpoint Anda (misalnya, <code>https://&lt;your-domain&gt;/api/auth/dodopayments/webhooks</code>) di Dasbor Dodo Payments dan atur di file <code>.env</code> Anda:
...
<AccordionGroup>
  <Accordion title="Opsi Plugin">
    - <b>client</b> (diperlukan): Instance klien DodoPayments -{" "}
    <b>createCustomerOnSignUp</b> (opsional): Buat pelanggan secara otomatis saat pengguna
    mendaftar - <b>use</b> (diperlukan): Array plugin yang diaktifkan (checkout,
    portal, usage, webhooks)
  </Accordion>
  <Accordion title="Opsi Plugin Checkout">
    - <b>products</b>: Array produk atau fungsi async yang mengembalikan produk -{" "}
    <b>successUrl</b>: URL untuk mengalihkan setelah pembayaran berhasil -{" "}
...
<AccordionGroup>
  <Accordion title="Masalah Umum">
    ...
    diatur di Dasbor Dodo Payments. - <b>Pelanggan tidak dibuat</b>: Konfirmasi <code>createCustomerOnSignUp</code> diatur ke <code>true</code>.
  </Accordion>
  <Accordion title="Praktik Terbaik">
    - Gunakan variabel lingkungan untuk semua rahasia dan kunci. - Uji di <code>test_mode</code> sebelum beralih ke <code>live_mode</code>. - Catat
    peristiwa webhook untuk debugging dan audit.
  </Accordion>
</AccordionGroup>
