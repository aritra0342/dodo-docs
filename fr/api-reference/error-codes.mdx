...
- `DUPLICATE_LINE_ITEMS_IN_REQUEST`
  ...
  - **Message :** Identifiants d'articles en double spécifiés dans le tableau des articles
...
- `INVALID_TAX_ID`
  - **Déclencheur :** Validation échouée de la TVA/GST/TIN
...
- `LINE_ITEM_REFUND_AMOUNT_TOO_HIGH`
  ...
  - **Message :** L'article de ligne {id} a demandé un montant de remboursement incluant la taxe de {amount} qui est supérieur au montant payé {amount}

- `LINE_ITEM_REFUND_AMOUNT_TOO_LOW`
  - **Déclencheur :** Montant de remboursement en dessous du seuil minimum
  - **Message :** L'article de ligne {id} a demandé un montant de remboursement de {amount} qui est trop bas
...
- `PAYMENT_NOT_SUCCEEDED`
  - **Déclencheur :** Tentative de rembourser/traiter un paiement non réussi
...
- `UNSUPPORTED_COUNTRY`
  - **Déclencheur :** Géographie non encore prise en charge
