...
1. Vous créez un abonnement avec l'objet `on_demand` pour autoriser une méthode de paiement et éventuellement collecter un frais initial.
...
<Accordion title="Paramètres du corps de la requête">
  ...
  <ParamField body="on_demand.product_price" type="integer">
    Montant de la charge initiale (dans la plus petite unité monétaire). Si spécifié, cette valeur remplace le prix d'origine du produit défini lors de la création du produit. Si omis, le prix stocké du produit est utilisé. Exemple : pour facturer 1,00 $, passez <code>100</code>.
  </ParamField>

  <ParamField body="on_demand.product_currency" type="string">
    Remplacement de devise optionnel pour la charge initiale. Par défaut, c'est la devise du produit.
  </ParamField>

  <ParamField body="on_demand.product_description" type="string">
    Remplacement de description optionnel pour la facturation et les lignes d'articles.
  </ParamField>
...
<AccordionGroup>
  <Accordion title="Paramètres du corps de la requête de charge">
    ...
    <ParamField body="product_currency" type="string">
      Remplacement de devise optionnel pour la charge.
    </ParamField>

    <ParamField body="product_description" type="string">
      Remplacement de description optionnel pour cette charge.
    </ParamField>
...
<Warning>
Les modèles de réessai en rafale peuvent être signalés comme frauduleux ou comme des tests de carte suspects par nos systèmes de risque et nos processeurs. Évitez les réessais groupés ; suivez le calendrier de retour et les conseils d'alignement temporel ci-dessous.
</Warning>
