...
<Warning>
Gérez toujours l'événement `checkout.error` pour offrir une bonne expérience utilisateur en cas de problème.
</Warning>
...
- Vérifiez que `elementId` correspond à l'`id` d'un `div` qui existe réellement dans le DOM.
...
Apple Pay permet aux clients de finaliser des paiements rapidement et en toute sécurité en utilisant leurs méthodes de paiement enregistrées. Lorsqu'il est activé, les clients peuvent lancer la fenêtre modale Apple Pay directement depuis le superposition de paiement sur les appareils pris en charge.
...
<Steps>
<Step title="Téléchargez et téléchargez le fichier d'association de domaine Apple Pay">
...
|---------|-----------------|------------------|
| Profondeur d'intégration | Entièrement intégré dans la page | Modale au-dessus de la page |
...
<Tip>
Utilisez **le paiement en ligne** lorsque vous souhaitez un contrôle maximal sur l'expérience de paiement et une intégration de marque transparente. Utilisez **la superposition de paiement** pour une intégration plus rapide avec des modifications minimales de vos pages existantes.
</Tip>
...
<Card title="Superposition de paiement" icon="layer-group" href="/developer-resources/overlay-checkout">
    Utilisez la superposition de paiement pour une intégration rapide basée sur une modale.
</Card>
