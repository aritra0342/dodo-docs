...
| Option | Type | Requis | Description |
|--------|------|----------|-------------|
| `checkoutUrl` | `string` | Oui | URL de la session de paiement. |
| `elementId` | `string` | Oui | L'`id` de l'élément DOM où le paiement doit être rendu. |
...
| `options.manualRedirect` | `boolean` | Non | Lorsqu'il est activé, le paiement ne redirigera pas automatiquement après la complétion. Au lieu de cela, vous recevrez les événements `checkout.status` et `checkout.redirect_requested` pour gérer la redirection vous-même. |
...
Lors de l'utilisation de `manualRedirect`, gérez la finalisation du paiement dans votre callback `onEvent` :
...
| `tax` | Le montant de la taxe calculé. En mode `inline`, cela se met à jour dynamiquement au fur et à mesure que l'utilisateur interagit avec les champs d'adresse. |
...
Vous pouvez personnaliser l'apparence du paiement en passant un `themeConfig` objet dans le paramètre `options` lors de l'ouverture du paiement. La configuration du thème prend en charge les modes clair et sombre, vous permettant de personnaliser les couleurs, les bordures, le texte, les boutons et le rayon des bordures.
...
Le SDK fournit des informations détaillées sur les erreurs via le système d'événements. Implémentez toujours une gestion appropriée des erreurs dans votre `onEvent` callback :
...
<Warning>
Veillez toujours à gérer l'événement `checkout.error` pour offrir une bonne expérience utilisateur en cas de problème.
</Warning>
...
<Steps>
<Step title="Télécharger et télécharger le fichier d'association de domaine Apple Pay">
...
<Step title="Demander l'activation d'Apple Pay">
...
<Step title="Vérifier la disponibilité d'Apple Pay">
...
|---------|-----------------|------------------|
| Profondeur d'intégration | Entièrement intégré dans la page | Modal sur la page |
...
<Tip>
Utilisez le **paiement en ligne** lorsque vous souhaitez un contrôle maximal sur l'expérience de paiement et une intégration de marque transparente. Utilisez le **paiement en superposition** pour une intégration plus rapide avec des modifications minimales de vos pages existantes.
</Tip>
...
<Card title="Superposition de Paiement" icon="layer-group" href="/developer-resources/overlay-checkout">
    Utilisez la superposition de paiement pour une intégration rapide basée sur une modalité.
</Card>
