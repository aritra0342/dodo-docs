...
<Note>
Nécessite Node.js 18 ou supérieur. Le `mcp-remote` package gère la connexion au serveur MCP distant.
</Note>
...
<Step title="Ouvrir les paramètres de Cursor">
    Naviguez vers **Paramètres de Cursor** > **Fonctionnalités** > **Modèle de Protocole de Contexte** ou appuyez sur `Cmd/Ctrl + Shift + P` et recherchez "Paramètres MCP".
</Step>
...
<Tab title="Claude Desktop">
Configurez le serveur Dodo Payments MCP dans Claude Desktop pour permettre un accès conversationnel à vos données de paiement.
<Steps>
...
Configurez le serveur Dodo Payments MCP dans Cline (VS Code) pour permettre un accès conversationnel à vos données de paiement.
<Steps>
