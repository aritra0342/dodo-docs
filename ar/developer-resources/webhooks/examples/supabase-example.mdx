...
4. قم بتمكين هذه الأحداث الاشتراكية:
   - `subscription.active`
...
2. **التحقق من التحمل** - يبحث عن معرف webhook لمنع المعالجة المكررة
3. **تسجيل الحدث** - يخزن بيانات webhook الخام في جدول `webhook_events`
4. **معالجة التحديثات** - ينشئ أو يحدث العملاء والاشتراكات عبر عميل Supabase
...
| خطأ في التحقق من JWT | نشر مع `--no-verify-jwt` العلم |
