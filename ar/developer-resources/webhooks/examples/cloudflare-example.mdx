...
قم بتحرير `wrangler.toml` وتعيين اسم العامل الخاص بك:
...
3. قم بتكوين عنوان URL الخاص بـ webhook كنقطة نهاية
...
3. **يسجل الحدث** - يخزن webhook الخام في جدول `webhook_events` لسجل التدقيق
