...
يوفر SDK معلومات تفصيلية عن الأخطاء من خلال نظام الأحداث. تأكد دائمًا من تنفيذ معالجة الأخطاء بشكل صحيح في `onEvent` callback:
...
2. **معالجة الأخطاء**: تأكد دائمًا من تنفيذ معالجة الأخطاء بشكل صحيح في callback الحدث
...
5. **عناوين URL صالحة**: استخدم دائمًا عناوين URL صالحة لصفحة الدفع من واجهة برمجة التطبيقات لإنشاء جلسة الدفع
6. **TypeScript**: استخدم TypeScript للحصول على أمان نوع أفضل وتجربة مطور أفضل
7. **حالات التحميل**: عرض حالات التحميل أثناء فتح صفحة الدفع لتحسين تجربة المستخدم
...
- عنوان URL لمجالك في الإنتاج (على سبيل المثال، `https://example.com`)
