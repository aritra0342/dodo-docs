...
بعد تحرير `convex.config.ts`، قم بتشغيل `npx convex dev` مرة واحدة لتوليد الأنواع اللازمة.
</Step>
...
- `DODO_PAYMENTS_API_KEY` - مفتاح API الخاص بك لمدفوعات Dodo
- `DODO_PAYMENTS_ENVIRONMENT` - تعيين إلى `test_mode` أو `live_mode`
...
<Warning>
قبل استخدام هذا الاستعلام، تأكد من تعريف المخطط المناسب في ملف `convex/schema.ts` أو تغيير الاستعلام ليتناسب مع المخطط الحالي لديك.
</Warning>
...
<Tab title="إعداد بوابة العملاء">
  <Info>
  استخدم هذه الوظيفة للسماح للعملاء بإدارة اشتراكاتهم وتفاصيلهم عبر بوابة العملاء لمدفوعات Dodo. يتم التعرف على العميل تلقائيًا عبر وظيفة `identify`.
  </Info>
  <CodeGroup>
...
<Tab title="إعداد معالج الويب هوك">
  <Info>
  استخدم هذا المعالج لاستلام ومعالجة أحداث الويب هوك من Dodo Payments بشكل آمن في تطبيق Convex الخاص بك. جميع معالجات الويب هوك تستقبل `ActionCtx` كأول معلمة، مما يتيح لك استخدام `ctx.runQuery()` و `ctx.runMutation()` للتفاعل مع قاعدة بياناتك.
  </Info>
  <CodeGroup>
...
<Info>
يتم التعرف على العميل تلقائيًا باستخدام وظيفة `identify` المكونة في إعدادات DodoPayments الخاصة بك. يجب أن ترجع هذه الوظيفة `dodoCustomerId` للعميل.
</Info>
