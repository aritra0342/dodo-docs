...
<Tip>
You can force a specific language on the checkout by setting the `force_language` parameter when creating a checkout session. See the [Checkout Sessions API](/developer-resources/checkout-session#forcing-a-language) for details.
</Tip>
