...
<Check>
Todos os pacotes necessários estão agora instalados.
</Check>
</Step>
</Steps>
...
Adicione estas ao seu arquivo <code>.env</code>:
...
<Warning>
Nunca comite chaves de API ou segredos no controle de versão.
</Warning>
</Step>

<Step title="Configurar integração do lado do servidor">
...
<Step title="Configurar integração do lado do cliente">
...
<Info>
  Ao contrário do método legado <code>checkout</code>, <code>checkoutSession</code>
  não requer informações de faturamento antecipadamente, pois serão coletadas do usuário
  na página de checkout. Você ainda pode substituir isso passando a
  chave <code>billing</code> no argumento.
</Info>

<Info>
  Semelhante ao método legado <code>checkout</code>,
  <code>checkoutSession</code> pega o e-mail e o nome do cliente de sua
  sessão better-auth, no entanto, você pode substituí-lo passando o
  objeto <code>customer</code> com e-mail e nome.
</Info>

<Info>
  Você pode passar as mesmas opções no argumento como o corpo da requisição para [Criar
  Sessão de Checkout](https://docs.dodopayments.com/api-reference/checkout-sessions/create)
  endpoint.
</Info>
...
<Warning>
  O método <code>authClient.dodopayments.checkout</code> está obsoleto. Use
  <code>checkoutSession</code> em vez disso para novas implementações.
</Warning>
...
<Tip>
  Timestamps mais antigos que uma hora ou mais de cinco minutos no futuro são
...
<Info>
  O plugin de webhooks processa eventos de pagamento em tempo real dos Pagamentos Dodo com
  verificação de assinatura segura. O endpoint padrão é <code>/api/auth/dodopayments/webhooks</code>.
</Info>

<Steps>
<Step title="Gerar e definir segredo do webhook">
Gere um segredo de webhook para sua URL de endpoint (por exemplo, <code>https://&lt;seu-domínio&gt;/api/auth/dodopayments/webhooks</code>) no Painel Dodo Payments e defina-o em seu arquivo <code>.env</code>:
...
<Step title="Manipular eventos de webhook">
...
<AccordionGroup>
  <Accordion title="Opções do Plugin">
    - <b>client</b> (obrigatório): Instância do cliente DodoPayments -{" "}
    <b>createCustomerOnSignUp</b> (opcional): Criar clientes automaticamente ao se inscrever - <b>use</b> (obrigatório): Array de plugins a serem habilitados (checkout,
    portal, uso, webhooks)
  </Accordion>
  <Accordion title="Opções do Plugin de Checkout">
    ...
    <b>authenticatedUsersOnly</b>: Exigir autenticação do usuário (padrão: false)
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Problemas Comuns">
    - <b>Chave de API inválida</b>: Verifique sua <code>DODO_PAYMENTS_API_KEY</code> em <code>.env</code>. -{" "}
    <b>Desvio de assinatura do webhook</b>: Certifique-se de que o segredo do webhook corresponda ao definido no Painel Dodo Payments. - <b>Cliente não criado</b>: Confirme que <code>createCustomerOnSignUp</code> está definido como <code>true</code>.
  </Accordion>
  <Accordion title="Melhores Práticas">
    - Use variáveis de ambiente para todos os segredos e chaves. - Teste em <code>test_mode</code> antes de mudar para <code>live_mode</code>. - Registre eventos de webhook para depuração e auditoria.
  </Accordion>
</AccordionGroup>
