...
1. **Design Responsivo**: Certifique-se de que seu elemento contêiner tenha largura e altura suficientes. O iframe geralmente se expande para preencher seu contêiner.
2. **Sincronização**: Use o evento `checkout.breakdown` para manter seu resumo de pedido personalizado ou tabelas de preços sincronizadas com o que o usuário vê no quadro de checkout.
...
<Info>
Para implementações em modo escuro, é recomendado usar `#0d0d0d` como a cor de fundo para uma integração visual ideal com o quadro de checkout inline.
</Info>
...
<Step title="Verificar a disponibilidade do Apple Pay">
...
<Warning>
O Apple Pay deve ser habilitado para seu domínio antes de aparecer como uma opção de pagamento em produção. Entre em contato com o suporte antes de ir ao vivo se você planeja oferecer o Apple Pay.
</Warning>
...
<Tip>
Use **checkout inline** quando você quiser controle máximo sobre a experiência de checkout e branding sem costura. Use **checkout overlay** para uma integração mais rápida com mudanças mínimas em suas páginas existentes.
</Tip>
