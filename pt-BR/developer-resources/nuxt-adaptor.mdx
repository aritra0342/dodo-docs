...
<CardGroup cols={2}>
<Card title="Rota da API de Checkout" icon="cart-shopping" href="#checkout-api-route">
    Integre o checkout dos pagamentos Dodo em seu aplicativo Nuxt usando uma rota de servidor.
</Card>
<Card title="Rota da API do Portal do Cliente" icon="user" href="#customer-portal-api-route">
    Permita que os clientes gerenciem assinaturas e detalhes através de uma rota de servidor Nuxt.
</Card>
<Card title="Rota da API de Webhooks" icon="bell" href="#webhook-api-route">
...
<Steps>
<Step title="Instale o módulo Nuxt">
...
<Step title="Registre o módulo em nuxt.config.ts">
Adicione <code>@dodopayments/nuxt</code> ao seu array de <code>modules</code> e configure-o:
...
<Info>
Todas as integrações de pagamentos Dodo no Nuxt são tratadas através de rotas de servidor no diretório <code>server/routes/api/</code>.
</Info>

<Tabs>
<Tab title="Rota da API de Checkout">
<Info>
...
<Tab title="Rota da API do Portal do Cliente">
<Info>
Crie uma rota GET para permitir que os clientes acessem seu portal. Aceita <code>customer_id</code> como um parâmetro de consulta.
</Info>
<CodeGroup>
...
<Tab title="Rota da API de Webhook">
<Info>
Crie uma rota POST para receber e processar eventos de webhook dos pagamentos Dodo de forma segura.
</Info>
<CodeGroup>
...
<Warning>
Se <code>productId</code> estiver ausente, o manipulador retorna uma resposta 400. Parâmetros de consulta inválidos também resultam em uma resposta 400.
</Warning>
...
As sessões de checkout fornecem uma experiência de checkout hospedada mais segura que lida com todo o fluxo de pagamento para compras únicas e assinaturas com controle total de personalização.
