...
O SDK fornece informações detalhadas sobre erros através do sistema de eventos. Sempre implemente um tratamento de erros adequado em seu `onEvent` callback:
...
3. **Modo de teste**: Use `test` durante o desenvolvimento e mude para `live` apenas quando estiver pronto para produção
...
7. **Estados de carregamento**: Mostre estados de carregamento enquanto o checkout está sendo aberto para melhorar a UX
