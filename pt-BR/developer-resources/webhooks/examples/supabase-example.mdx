...
> **Nota:** Esta implementação demonstra o tratamento de três eventos principais de assinatura (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) com campos mínimos. Você pode facilmente estendê-la para suportar tipos de eventos e campos adicionais com base em suas necessidades.
...
5. **Lida com erros** - Registra falhas e marca o evento para reprocessamento
...
<Note>
A `--no-verify-jwt` flag é necessária porque os webhooks não incluem tokens JWT. A segurança é fornecida pela verificação da assinatura do webhook.
</Note>
