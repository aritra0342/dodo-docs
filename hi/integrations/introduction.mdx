---
sidebarTitle: परिचय
title: बाहरी एकीकरण
description: अपने पसंदीदा उपकरणों और प्लेटफार्मों के साथ Dodo Payments को कनेक्ट
  करें ताकि कार्यप्रवाह को स्वचालित किया जा सके और आपके भुगतान अनुभव को बढ़ाया
  जा सके।
icon: book
---

## अवलोकन

Dodo Payments लोकप्रिय प्लेटफार्मों और उपकरणों के साथ स्वदेशी एकीकरण प्रदान करता है ताकि आप अपने भुगतान संचालन को सुव्यवस्थित कर सकें और अपने कार्यप्रवाह को स्वचालित कर सकें। चाहे आपको अपनी टीम को सूचनाएँ भेजने, ग्राहक डेटा को अपने CRM में समन्वयित करने, ईमेल अभियानों को ट्रिगर करने, या अपने पसंदीदा विकास उपकरणों के साथ कनेक्ट करने की आवश्यकता हो, हमारे पास आपकी आवश्यकताओं के लिए एक एकीकरण है।

हमारे एकीकरण को सेट अप करना आसान बनाने के लिए डिज़ाइन किया गया है और इसके लिए न्यूनतम कॉन्फ़िगरेशन की आवश्यकता होती है। अधिकांश एकीकरण को आपके Dodo Payments डैशबोर्ड से केवल कुछ क्लिक में सीधे कनेक्ट किया जा सकता है, जिससे आप तुरंत अपने भुगतान कार्यप्रवाह को स्वचालित करना शुरू कर सकते हैं।

<Info>
नीचे उपलब्ध एकीकरणों को ब्राउज़ करें या जल्दी से उस एकीकरण को खोजने के लिए खोज का उपयोग करें जिसकी आपको आवश्यकता है। प्रत्येक एकीकरण में आपको शुरू करने के लिए चरण-दर-चरण सेटअप निर्देश शामिल हैं।
</Info>

## एकीकरण के लाभ

<CardGroup cols={2}>
  <Card title="वास्तविक समय की सूचनाएँ" icon="bell">
    महत्वपूर्ण भुगतान घटनाओं के बारे में सूचित रहें जैसे ही वे होती हैं, यह सुनिश्चित करते हुए कि आप अपने व्यवसाय के बारे में महत्वपूर्ण अपडेट कभी न चूकें।
  </Card>
  <Card title="स्वचालित कार्यप्रवाह" icon="bolt">
    भुगतान घटनाओं के आधार पर स्वचालित रूप से क्रियाएँ ट्रिगर करके मैनुअल कार्यों को कम करें, समय बचाएं और त्रुटियों को कम करें।
  </Card>
  <Card title="बढ़ी हुई दृश्यता" icon="chart-bar">
    अपने पसंदीदा उपकरणों के बीच केंद्रीकृत सूचनाओं और रिपोर्टिंग के साथ अपने भुगतान संचालन में बेहतर अंतर्दृष्टि प्राप्त करें।
  </Card>
  <Card title="सुधरी हुई ग्राहक अनुभव" icon="face-smile">
    अपने मौजूदा उपकरणों और कार्यप्रवाहों के साथ एकीकृत करके निर्बाध भुगतान अनुभव प्रदान करें।
  </Card>
</CardGroup>

<Tip>
प्रत्येक एकीकरण में आपको जल्दी शुरू करने में मदद करने के लिए विस्तृत सेटअप निर्देश और उदाहरण कॉन्फ़िगरेशन शामिल हैं।
</Tip>

## उपलब्ध एकीकरण

Dodo Payments को निम्नलिखित प्लेटफार्मों और उपकरणों के साथ कनेक्ट करें:

<CardGroup cols={2}>
  <Card title="N8N" icon="diagram-project" href="/integrations/n8n">
    Dodo Payments को N8N वेबहुक के माध्यम से 5000+ ऐप्स से कनेक्ट करें, जिससे अनलिमिटेड ऑटोमेशन संभावनाएं मिलती हैं, चाहे आप स्वयं-होस्टेड वर्कफ़्लो का उपयोग करें या क्लाउड वर्कफ़्लो का।
  </Card>
  <Card title="Slack" icon="slack" href="/integrations/slack">
    अपने Slack कार्यक्षेत्र में सीधे अपने भुगतानों के बारे में वास्तविक समय की सूचनाएं प्राप्त करें।
  </Card>
  <Card title="HubSpot" icon="hubspot" href="/integrations/hubspot">
    Dodo Payments घटनाओं से HubSpot में संपर्क, कंपनियों और सौदों को स्वचालित रूप से बनाएं और अपडेट करें।
  </Card>
  <Card title="WooCommerce Plugin" icon="wordpress" href="/integrations/woocommerce-plugin">
    अपने WooCommerce स्टोर के साथ Dodo Payments को सहजता से एकीकृत करें ताकि वैश्विक भुगतान स्वीकार कर सकें।
  </Card>
  <Card title="Discord" icon="discord" href="/integrations/discord">
    अपने Discord चैनलों पर वास्तविक समय की Dodo Payments सूचनाएं भेजें।
  </Card>
  <Card title="Microsoft Teams" icon="microsoft" href="/integrations/microsoft-teams">
    समृद्ध एडेप्टिव कार्ड के साथ Microsoft Teams चैनलों पर Dodo Payments सूचनाएं भेजें।
  </Card>
  <Card title="Resend" icon="envelope" href="/integrations/resend">
    Dodo Payments घटनाओं के आधार पर Resend के माध्यम से लेनदेन ईमेल भेजें, जिसमें उच्च डिलीवरी क्षमता हो।
  </Card>
  <Card title="SendGrid" icon="inbox" href="/integrations/sendgrid">
    Dodo Payments घटनाओं के आधार पर SendGrid के Mail Send API का उपयोग करके लेनदेन ईमेल भेजें।
  </Card>
  <Card title="Customer.io" icon="chart-area" href="/integrations/customer-io">
    ईमेल ऑटोमेशन और ग्राहक यात्रा को ट्रिगर करने के लिए Dodo Payments घटनाओं को Customer.io पर भेजें।
  </Card>
  <Card title="Loops" icon="infinity" href="/integrations/loops">
    Dodo Payments घटनाओं के आधार पर लेनदेन ईमेल भेजें और ग्राहक संचार प्रबंधित करें।
  </Card>
  <Card title="AutoSend" icon="paper-plane" href="/integrations/autosend">
    AutoSend के ईमेल API का उपयोग करके अपने Dodo Payments लेनदेन, रिफंड और भुगतान घटनाओं के लिए स्वचालित लेनदेन ईमेल भेजें।
  </Card>
  <Card title="Segment" icon="chart-line" href="/integrations/segment">
    विश्लेषण और ग्राहक डेटा प्लेटफ़ॉर्म एकीकरण के लिए Dodo Payments घटनाओं को Segment पर भेजें।
  </Card>
  <Card title="DataFast" icon="chart-bar" href="/integrations/datafast">
    DataFast विश्लेषण के साथ विपणन चैनलों को राजस्व सौंपें। ट्रैक करें कि कौन से ट्रैफ़िक स्रोत भुगतान करने वाले ग्राहकों को लाते हैं और अपनी वृद्धि को अनुकूलित करें।
  </Card>
  <Card title="Dub" icon="link" href="/integrations/dub">
    जब ग्राहक खरीदारी पूरी करते हैं तो Dub में बिक्री रूपांतरण घटनाओं को ट्रैक करें। विपणन अभियानों और रेफरल कार्यक्रमों की ROI मापें।
  </Card>
  <Card title="Inngest" icon="cloud" href="/integrations/inngest">
    Dodo Payments घटनाओं के आधार पर Inngest में सर्वर रहित कार्य और बैकग्राउंड जॉब्स को ट्रिगर करें।
  </Card>
  <Card title="Zapier" icon="diagram-project" href="/integrations/zapier">
    अनलिमिटेड ऑटोमेशन संभावनाओं के लिए Zapier के माध्यम से Dodo Payments को 5000+ ऐप्स से कनेक्ट करें।
  </Card>
  <Card title="Windmill" icon="wind" href="/integrations/windmill">
    कस्टम ऑटोमेशन के लिए Dodo Payments घटनाओं के आधार पर Windmill वर्कफ़्लो और स्क्रिप्ट को ट्रिगर करें।
  </Card>
  <Card title="Close CRM" icon="handshake" href="/integrations/close-crm">
    लीड ट्रैक करने और बिक्री पाइपलाइन प्रबंधित करने के लिए Dodo Payments डेटा को Close CRM के साथ समन्वयित करें।
  </Card>
  <Card title="Figma Plugin" icon="figma" href="/integrations/figma-plugin">
    Dodo Payments लाइसेंस कुंजी सत्यापन और सक्रियण के साथ Figma प्लगइन्स बनाएं।
  </Card>
  <Card title="Framer Plugin" icon="puzzle-piece" href="/integrations/framer-plugin">
    अनुकूलन योग्य चेकआउट अनुभवों के साथ अपने Framer प्रोजेक्ट्स में Dodo Payments चेकआउट कार्यक्षमता को एकीकृत करें।
  </Card>
  <Card title="Raycast Extension" icon="puzzle-piece" href="/integrations/raycast-extension">
    अपने कीबोर्ड से Dodo Payments संसाधनों को ब्राउज़ और प्रबंधित करने के लिए Raycast एक्सटेंशन स्थापित करें।
  </Card>
</CardGroup>

## एकीकरण श्रेणियाँ

हमारे एकीकरण को आपकी आवश्यकताओं को खोजने में मदद करने के लिए कई श्रेणियों में व्यवस्थित किया गया है:

- **संचार और सहयोग**: Slack, Discord, और Microsoft Teams में वास्तविक समय की सूचनाओं के साथ अपनी टीम को सूचित रखें
- **ईमेल सेवाएँ**: Resend, SendGrid, Customer.io, Loops, और AutoSend के साथ लेनदेन ईमेल और ग्राहक संचार को स्वचालित करें
- **CRM और बिक्री**: HubSpot और Close CRM के साथ भुगतान डेटा और ग्राहक जानकारी को समन्वयित करें
- **स्वचालन प्लेटफार्म**: कस्टम कार्यप्रवाह बनाने के लिए Zapier, N8N, Inngest, और Windmill के साथ कनेक्ट करें
- **विश्लेषण**: Segment और DataFast के साथ भुगतान घटनाओं को ट्रैक और विश्लेषण करें
- **ई-कॉमर्स**: अपने WooCommerce स्टोर में भुगतान को निर्बाध रूप से एकीकृत करें
- **डिज़ाइन उपकरण**: Figma और Framer के लिए प्लगइन्स और एक्सटेंशन बनाएं
- **डेवलपर उपकरण**: Raycast से Dodo Payments कार्यक्षमता तक पहुँचें

## क्या आपको एक अलग एकीकरण की आवश्यकता है?

क्या आप जिस प्लेटफार्म की तलाश कर रहे हैं वह नहीं दिख रहा है? हम उपयोगकर्ता फीडबैक के आधार पर लगातार नए एकीकरण जोड़ रहे हैं।

- **एक नया एकीकरण अनुरोध करें** Dodo Payments टीम से [support@dodopayments.com](mailto:support@dodopayments.com) पर संपर्क करके। हमें बताएं कि आप किस प्लेटफार्म को कनेक्ट करना चाहते हैं, और हमारी टीम आपके अनुरोध की समीक्षा करेगी।
- **यदि आपको तुरंत समाधान की आवश्यकता है तो हमारे वेबहुक सिस्टम का उपयोग करके एक कस्टम एकीकरण बनाएं।** हमारे वेबहुक दस्तावेज़ में आपको शुरू करने के लिए आवश्यक सभी विवरण प्रदान किए गए हैं।

<Card title="Webhook Documentation" icon="webhook" href="/developer-resources/webhooks">
    जानें कि हमारे वेबहुक सिस्टम का उपयोग करके कस्टम एकीकरण कैसे बनाएं।
</Card>