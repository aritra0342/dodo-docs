...
<Note>
  इस एकीकरण का उपयोग करने के लिए आपको डोडो पेमेंट्स खाता और API कुंजी की आवश्यकता है।
</Note>
...
<Check>
सभी आवश्यक पैकेज अब स्थापित हैं।
</Check>
</Step>
</Steps>
...
अपने <code>.env</code> फ़ाइल में ये जोड़ें:
...
<Warning>
कभी भी API कुंजी या रहस्यों को संस्करण नियंत्रण में कमिट न करें।
</Warning>
</Step>

<Step title="सर्वर-साइड एकीकरण सेट करें">
...
<Step title="क्लाइंट-साइड एकीकरण सेट करें">
...
<Info>
  नए एकीकरण के लिए <code>authClient.dodopayments.checkoutSession</code> को प्राथमिकता दें। पुरानी <code>checkout</code> विधि को हटा दिया गया है और इसे केवल पिछली संगतता के लिए रखा गया है।
</Info>
...
<Info>
  पुरानी <code>checkout</code> विधि के विपरीत, <code>checkoutSession</code>
  को अग्रिम में बिलिंग जानकारी की आवश्यकता नहीं होती है क्योंकि इसे चेकआउट पृष्ठ पर उपयोगकर्ता से लिया जाएगा। आप इसे <code>billing</code> कुंजी को तर्क में पास करके ओवरराइड कर सकते हैं।
</Info>

<Info>
  पुरानी <code>checkout</code> विधि के समान,
  <code>checkoutSession</code> ग्राहक के ईमेल और नाम को उनके
  बेहतर-ऑथ सत्र से लेता है, हालाँकि, आप इसे <code>customer</code> ऑब्जेक्ट को ईमेल और नाम के साथ पास करके ओवरराइड कर सकते हैं।
</Info>

<Info>
  आप तर्क में वही विकल्प पास कर सकते हैं जो [चेकआउट सत्र बनाने](https://docs.dodopayments.com/api-reference/checkout-sessions/create) एंडपॉइंट के लिए अनुरोध शरीर के रूप में हैं।
</Info>

<Note>
  लौटने वाला URL <code>successUrl</code> से लिया जाता है जो सर्वर प्लगइन में कॉन्फ़िगर किया गया है। आपको क्लाइंट पेलोड में <code>return_url</code> शामिल करने की आवश्यकता नहीं है।
</Note>

## पुराना चेकआउट (हटाया गया)

<Warning>
  <code>authClient.dodopayments.checkout</code> विधि हटा दी गई है। नए कार्यान्वयन के लिए <code>checkoutSession</code> का उपयोग करें।
</Warning>
...
<Tip>
  एक घंटे से पुराने या भविष्य में पांच मिनट से अधिक समय के टाइमस्टैम्प को उपयोग इनजेस्ट करते समय अस्वीकृत कर दिया जाता है।
</Tip>
<Tip>
  यदि आप उपयोग मीटरों की सूची बनाते समय <code>meter_id</code> छोड़ देते हैं, तो सभी मीटर लौटाए जाते हैं
  ग्राहक की सदस्यताओं से जुड़े होते हैं।
</Tip>

# वेबहुक

<Info>
  वेबहुक प्लगइन डोडो पेमेंट्स से वास्तविक समय के भुगतान इवेंट को सुरक्षित हस्ताक्षर सत्यापन के साथ प्रोसेस करता है। डिफ़ॉल्ट एंडपॉइंट है <code>/api/auth/dodopayments/webhooks</code>।
</Info>

<Steps>
<Step title="वेबहुक रहस्य उत्पन्न करें और सेट करें">
अपने एंडपॉइंट URL (जैसे, <code>https://&lt;your-domain&gt;/api/auth/dodopayments/webhooks</code>) के लिए डोडो पेमेंट्स डैशबोर्ड में एक वेबहुक रहस्य उत्पन्न करें और इसे अपने <code>.env</code> फ़ाइल में सेट करें:
...
<Step title="वेबहुक इवेंट को संभालें">
उदाहरण हैंडलर:
...
<AccordionGroup>
  <Accordion title="प्लगइन विकल्प">
    - <b>client</b> (आवश्यक): डोडो पेमेंट्स क्लाइंट उदाहरण - <b>createCustomerOnSignUp</b> (वैकल्पिक): उपयोगकर्ता साइनअप पर ग्राहकों को स्वचालित रूप से बनाना - <b>use</b> (आवश्यक): सक्षम करने के लिए प्लगइन्स की सूची (चेकआउट, पोर्टल, उपयोग, वेबहुक)
  </Accordion>
  <Accordion title="चेकआउट प्लगइन विकल्प">
    - <b>products</b>: उत्पादों की सूची या असिंक्रोनस फ़ंक्शन जो उत्पाद लौटाता है - <b>successUrl</b>: सफल भुगतान के बाद पुनर्निर्देशित करने के लिए URL - <b>authenticatedUsersOnly</b>: उपयोगकर्ता प्रमाणीकरण की आवश्यकता (डिफ़ॉल्ट: गलत)
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="सामान्य समस्याएँ">
    - <b>अमान्य API कुंजी</b>: अपने <code>DODO_PAYMENTS_API_KEY</code> को <code>.env</code> में दोबारा जांचें। - <b>वेबहुक हस्ताक्षर असंगतता</b>: सुनिश्चित करें कि वेबहुक रहस्य डोडो पेमेंट्स डैशबोर्ड में सेट किए गए के साथ मेल खाता है। - <b>ग्राहक नहीं बनाया गया</b>: पुष्टि करें कि <code>createCustomerOnSignUp</code> को <code>true</code> पर सेट किया गया है।
  </Accordion>
  <Accordion title="सर्वश्रेष्ठ प्रथाएँ">
    - सभी रहस्यों और कुंजियों के लिए पर्यावरण चर का उपयोग करें। - <code>live_mode</code> में स्विच करने से पहले <code>test_mode</code> में परीक्षण करें। - डिबगिंग और ऑडिटिंग के लिए वेबहुक इवेंट को लॉग करें।
  </Accordion>
</AccordionGroup>
