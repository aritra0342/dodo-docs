...
Puedes personalizar la apariencia del checkout pasando un `themeConfig` objeto en el parámetro `options` al abrir el checkout. La configuración del tema admite modos claro y oscuro, lo que te permite personalizar colores, bordes, texto, botones y radio de borde.
...
3. **Modo de prueba**: Usa `test` durante el desarrollo y cambia a `live` solo cuando estés listo para producción
4. **Manejo de eventos**: Maneja todos los eventos relevantes para una experiencia de usuario completa
5. **URLs válidas**: Siempre usa URLs de checkout válidas de la API de creación de sesiones de checkout
...
9. **Gestión de temporizadores**: Desactiva el temporizador (`showTimer: false`) si deseas manejar la expiración de la sesión manualmente
...
- Tu URL de dominio de producción (por ejemplo, `https://example.com`)
