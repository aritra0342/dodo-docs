...
<Note>
Requiere Node.js 18 o superior. El `mcp-remote` paquete maneja la conexión al servidor MCP remoto.
</Note>
...
1. **Borrar caché de autenticación de MCP**: `rm -rf ~/.mcp-auth`
2. **Reinicia tu aplicación cliente**
3. **Revisa los registros del cliente** para mensajes de error
...
<Steps>
<Step title="Abre la configuración de Cursor">
...
<Tab title="Claude Desktop">
Configura el servidor Dodo Payments MCP en Claude Desktop para habilitar el acceso conversacional a tus datos de pagos.
<Steps>
<Step title="Localiza el archivo de configuración">
...
Configura el servidor Dodo Payments MCP en Cline (VS Code) para habilitar el acceso conversacional a tus datos de pagos.
<Steps>
...
<Tip>
Ejecuta `npx dodopayments-mcp --describe-capabilities` para información detallada sobre capacidades.
</Tip>
...
3. **`invoke_api_endpoint`** - Ejecuta cualquier método con parámetros apropiados

<Info>
Las herramientas dinámicas se habilitan automáticamente para APIs con más de 50 métodos. Anula con `--tools=all` para incluir todas las herramientas.
</Info>
...
Al desplegar de forma remota, siempre requiere autenticación a través del encabezado `Authorization` o el encabezado `x-dodo-payments-api-key`.
...
Algunos clientes no soportan todas las características del esquema JSON. Usa `--describe-capabilities` para entender qué se soporta.
</Accordion>
</AccordionGroup>
