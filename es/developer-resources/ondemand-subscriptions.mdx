...
<Accordion title="Parámetros del cuerpo de la solicitud">
  ...
  <ParamField body="on_demand.adaptive_currency_fees_inclusive" type="boolean">
    Si es verdadero, incluye las tarifas de moneda adaptativa dentro de <code>product_price</code>. Si es falso, las tarifas se añaden por encima. Ignorado cuando la fijación de precios adaptativa está desactivada.
  </ParamField>
</Accordion>
</AccordionGroup>
...
<Warning>
Los patrones de reintento en ráfaga pueden ser marcados como fraudulentos o sospechosos de pruebas de tarjeta por nuestros sistemas de riesgo y procesadores. Evita reintentos agrupados; sigue el programa de retroceso y la guía de alineación de tiempo a continuación.
</Warning>
...
### Programa de reintentos sugerido (suscripciones)
...
- Alternativamente, si almacenas la última hora de pago exitosa `T`, programa el siguiente intento en `T + X days` para preservar la alineación de la hora del día.
...
- Utiliza un programador/cola que persista marcas de tiempo precisas; calcula el siguiente intento en el mismo desplazamiento de hora del día (por ejemplo, `T + 3 days` a la misma HH:MM).
- Mantén y referencia la última marca de tiempo de pago exitosa `T` para calcular el siguiente intento; no agrupes múltiples suscripciones en el mismo instante.
