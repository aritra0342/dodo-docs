...
Un producto puede ser un artículo único o un servicio basado en suscripción. Para crear un nuevo producto, envía un `POST` a la `/products` con detalles como nombre, descripción, precio, moneda y si es un producto recurrente.
...
Para modificar un producto existente, envía un `PATCH` a la `/products/{product_id}`. Puedes actualizar propiedades como nombre, precio y descripción mientras mantienes otros detalles sin cambios.
...
Una vez que se recibe la URL pre-firmada de la API, carga la imagen usando el `PUT` método. Esto asegura acceso seguro y temporal a AWS S3 para cargar la imagen.
