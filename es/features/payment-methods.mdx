...
Si bien todos los métodos de pago están disponibles automáticamente según la ubicación y la moneda del cliente, puedes restringir opcionalmente qué opciones de pago aparecen durante el proceso de pago utilizando el parámetro `allowed_payment_method_types`.
...
Si `allowed_payment_method_types` no se especifica, todos los métodos de pago elegibles estarán disponibles según la ubicación del cliente, la moneda y el tipo de transacción. Usa este parámetro solo cuando necesites restringir métodos de pago.
</Accordion>
...
- El cliente desea usar un método de pago diferente
