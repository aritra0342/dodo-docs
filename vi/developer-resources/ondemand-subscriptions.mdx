...
2. Sau đó, bạn tạo các khoản phí đối với đăng ký đó với số tiền tùy chỉnh bằng cách sử dụng điểm cuối thu phí chuyên dụng.
...
<Accordion title="Tham số Nội dung Yêu cầu">
  ...
  <ParamField body="customer" type="object" required>
    Gắn một khách hàng hiện có hoặc cung cấp thông tin khách hàng.
  </ParamField>
  ...
  <ParamField body="on_demand.product_price" type="integer">
    Số tiền thu phí ban đầu (tính bằng đơn vị tiền tệ nhỏ nhất). Nếu được chỉ định, giá trị này sẽ ghi đè lên giá gốc của sản phẩm được thiết lập trong quá trình tạo sản phẩm. Nếu bị bỏ qua, giá đã lưu của sản phẩm sẽ được sử dụng. Ví dụ: để tính phí $1.00, hãy truyền <code>100</code>.
  </ParamField>
  ...
  <ParamField body="on_demand.product_description" type="string">
    Ghi đè mô tả tùy chọn cho thanh toán và các mục dòng.
  </ParamField>
...
<AccordionGroup>
  <Accordion title="Tham số nội dung yêu cầu thu phí">
...
<Warning>
Các mẫu thử lại bùng nổ có thể bị đánh dấu là gian lận hoặc thử nghiệm thẻ nghi ngờ bởi các hệ thống và nhà xử lý rủi ro của chúng tôi. Tránh các lần thử lại cụm; tuân theo lịch trình lùi và hướng dẫn căn thời gian bên dưới.
</Warning>
...
- Ví dụ: Nếu khách hàng bắt đầu một thử nghiệm hoặc mệnh lệnh vào lúc 1:10 chiều hôm nay, hãy lên lịch các lần thử lại tiếp theo vào lúc 1:10 chiều vào các ngày tiếp theo theo lịch lùi của bạn (ví dụ: +3 ngày → 1:10 chiều, +7 ngày → 1:10 chiều).
...
<Note>
Để có danh sách đầy đủ các lý do từ chối và liệu chúng có thể được sửa bởi người dùng hay không, xem tài liệu
...
Gọi điểm cuối thu phí với một `product_price` nhỏ (ví dụ: `100`) và xác minh rằng bạn nhận được `payment.succeeded`.
</Step>
