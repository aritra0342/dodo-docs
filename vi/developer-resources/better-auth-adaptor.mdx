...
Thêm những điều này vào tệp <code>.env</code> của bạn:
...
<Warning>
Không bao giờ cam kết khóa API hoặc bí mật vào kiểm soát phiên bản.
</Warning>
</Step>

<Step title="Thiết lập tích hợp phía máy chủ">
...
<Step title="Thiết lập tích hợp phía khách hàng">
...
<Info>
  Ưu tiên sử dụng <code>authClient.dodopayments.checkoutSession</code> cho các
  tích hợp mới. Phương thức <code>checkout</code> cũ đã bị ngừng sử dụng và chỉ
...
<Info>
  ...
  không yêu cầu thông tin thanh toán trước vì nó sẽ được lấy từ người dùng
  tại trang thanh toán. Bạn vẫn có thể ghi đè nó bằng cách truyền khóa
...
<Info>
  ...
  better-auth của họ, tuy nhiên, bạn có thể ghi đè nó bằng cách truyền
  đối tượng <code>customer</code> với email và tên.
</Info>

<Info>
  Bạn có thể truyền cùng một tùy chọn trong đối số như thân yêu cầu cho [Tạo
  Phiên thanh toán](https://docs.dodopayments.com/api-reference/checkout-sessions/create)
  điểm cuối.
</Info>

<Note>
  URL trả về được lấy từ <code>successUrl</code> được cấu hình trong
  plugin máy chủ. Bạn không cần phải bao gồm <code>return_url</code> trong
...
<Warning>
  Phương thức <code>authClient.dodopayments.checkout</code> đã bị ngừng sử dụng. Sử dụng
  <code>checkoutSession</code> thay thế cho các triển khai mới.
</Warning>
...
<Tip>
  Các dấu thời gian cũ hơn một giờ hoặc hơn năm phút vào tương lai sẽ
  bị từ chối khi ghi lại mức sử dụng.
</Tip>
<Tip>
  Nếu bạn bỏ qua <code>meter_id</code> khi liệt kê các đồng hồ sử dụng, tất cả các đồng hồ
  liên kết với các đăng ký của khách hàng sẽ được trả về.
</Tip>

# Webhooks

<Info>
  Plugin webhooks xử lý các sự kiện thanh toán theo thời gian thực từ Dodo Payments với
  xác minh chữ ký an toàn. Điểm cuối mặc định là <code>/api/auth/dodopayments/webhooks</code>.
</Info>

<Steps>
<Step title="Tạo và thiết lập bí mật webhook">
Tạo một bí mật webhook cho URL điểm cuối của bạn (ví dụ: <code>https://&lt;your-domain&gt;/api/auth/dodopayments/webhooks</code>) trong Bảng điều khiển Dodo Payments và thiết lập nó trong tệp <code>.env</code> của bạn:
...
<Step title="Xử lý các sự kiện webhook">
...
<AccordionGroup>
  <Accordion title="Tùy chọn Plugin">
    - <b>client</b> (bắt buộc): Thể hiện khách hàng DodoPayments -{" "}
    <b>createCustomerOnSignUp</b> (tùy chọn): Tự động tạo khách hàng khi người dùng
    đăng ký - <b>use</b> (bắt buộc): Mảng các plugin để kích hoạt (thanh toán,
    cổng, sử dụng, webhooks)
  </Accordion>
  <Accordion title="Tùy chọn Plugin Thanh toán">
    - <b>products</b>: Mảng các sản phẩm hoặc hàm bất đồng bộ trả về sản phẩm -{" "}
...
<AccordionGroup>
  <Accordion title="Vấn đề Thường gặp">
    - <b>Khóa API không hợp lệ</b>: Kiểm tra lại khóa <code>DODO_PAYMENTS_API_KEY</code> của bạn trong <code>.env</code>. -{" "}
    <b>Không khớp chữ ký webhook</b>: Đảm bảo bí mật webhook khớp với bí mật đã
    thiết lập trong Bảng điều khiển Dodo Payments. - <b>Khách hàng không được tạo</b>: Xác nhận <code>createCustomerOnSignUp</code> được đặt thành <code>true</code>.
  </Accordion>
  <Accordion title="Thực hành Tốt nhất">
    - Sử dụng biến môi trường cho tất cả các bí mật và khóa. - Kiểm tra trong <code>test_mode</code> trước khi chuyển sang <code>live_mode</code>. - Ghi lại các sự kiện webhook để gỡ lỗi và kiểm toán.
  </Accordion>
</AccordionGroup>
