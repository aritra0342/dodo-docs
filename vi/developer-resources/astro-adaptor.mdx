...
<Info>
Tất cả các ví dụ đều giả định rằng bạn đang sử dụng Astro App Router.
</Info>

<Tabs>
<Tab title="Xử lý Thanh Toán">
  <Info>
  Sử dụng trình xử lý này để tích hợp thanh toán Dodo Payments vào ứng dụng Astro của bạn. Hỗ trợ các luồng thanh toán tĩnh (GET), động (POST) và phiên (POST).
  </Info>
  <CodeGroup>
...
<Tab title="Xử lý Webhook">
  <Info>
  Sử dụng trình xử lý này để nhận và xử lý các sự kiện webhook của Dodo Payments một cách an toàn trong ứng dụng Astro của bạn.
  </Info>
  <CodeGroup>
