...
**Ví dụ:** Đếm các cuộc gọi API nơi `user_tier = "premium"` VÀ `endpoint = "/api/v2/users"`
</Tab>
...
**Ví dụ:** Đếm các sự kiện nơi `method = "POST"` HOẶC `method = "PUT"` HOẶC `method = "DELETE"`
</Tab>
</Tabs>
...
**Ví dụ**: Đặt `$0.50` mỗi đơn vị có nghĩa là:
...
Cấu hình lọc (sử dụng logic AND):
