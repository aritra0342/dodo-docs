...
1. Đi đến [Bảng điều khiển DodoPayments](https://app.dodopayments.com) → Nhà phát triển → Webhooks
...
3. **Ghi lại sự kiện** - Lưu trữ webhook thô trong bảng `webhook_events` để theo dõi
...
1. Chọn trang web của bạn → tab **Functions**
2. Nhấp vào hàm `webhook`
3. Xem nhật ký theo thời gian thực và lịch sử gọi hàm
...
| Lỗi kết nối cơ sở dữ liệu | Xác minh chuỗi kết nối Neon và sử dụng kết nối được phân phối |
| Không tìm thấy hàm (404) | Chạy `netlify init` và `npm run deploy` lại |
| Biến môi trường không khả dụng | Thiết lập biến trong CLI hoặc bảng điều khiển, sau đó triển khai lại |
