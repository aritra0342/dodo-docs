...
1. Đi đến [Bảng điều khiển Supabase](https://supabase.com/dashboard)
2. Mở Trình soạn thảo SQL
...
Chức năng đã được thiết lập trong `functions/webhook/index.ts` - chỉ cần triển khai nó:
...
<Note>
Cờ `--no-verify-jwt` là cần thiết vì webhooks không bao gồm mã thông báo JWT. Bảo mật được cung cấp bởi xác minh chữ ký webhook.
</Note>
...
| Không tìm thấy chức năng | Xác minh ref dự án có đúng và chức năng đã được triển khai |
