...
<Tab title="Thiết lập Xử lý Webhook">
  <Info>
  Sử dụng trình xử lý này để nhận và xử lý các sự kiện webhook của Dodo Payments một cách an toàn trong ứng dụng Convex của bạn. Tất cả các trình xử lý webhook nhận `ActionCtx` như tham số đầu tiên, cho phép bạn sử dụng `ctx.runQuery()` và `ctx.runMutation()` để tương tác với cơ sở dữ liệu của bạn.
  </Info>
  <CodeGroup>
