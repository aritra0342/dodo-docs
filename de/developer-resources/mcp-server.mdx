...
<Tabs>
<Tab title="Cursor">
Fügen Sie hinzu `~/.cursor/mcp.json`:
...
<Tab title="Windsurf">
Fügen Sie hinzu `~/.codeium/windsurf/mcp_config.json`:
...
Nachdem Sie geklickt haben, setzen Sie Ihre Umgebungsvariablen in Cursors `mcp.json` über **Cursor-Einstellungen** > **Tools & MCP** > **Neuer MCP-Server**.
...
Nachdem Sie geklickt haben, setzen Sie Ihre Umgebungsvariablen in VS Codes `mcp.json` über **Befehls-Palette** > **MCP: Benutzerkonfiguration öffnen**.
...
| `DODO_PAYMENTS_WEBHOOK_KEY` | Ihr Webhook-Signierungsschlüssel | Nein |
...
<Tip>
Führen Sie `npx dodopayments-mcp --describe-capabilities` für detaillierte Fähigkeitsinformationen aus.
</Tip>
...
2. **`get_api_endpoint_schema`** - Holt detaillierte Schemata für eine bestimmte Methode
...
Bei der Bereitstellung in der Ferne ist immer eine Authentifizierung über den `Authorization` Header oder den `x-dodo-payments-api-key` Header erforderlich.
