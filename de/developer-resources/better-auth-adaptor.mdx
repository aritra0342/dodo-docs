...
<Warning>
Verpflichten Sie niemals API-Schlüssel oder Geheimnisse in die Versionskontrolle.
</Warning>
</Step>
...
<Info>
  ...
  Integrationen. Die veraltete <code>checkout</code>-Methode ist nicht mehr empfohlen und wird
...
<Info>
  Ähnlich wie bei der veralteten <code>checkout</code>-Methode
  nimmt <code>checkoutSession</code> die E-Mail und den Namen des Kunden aus seiner
...
<Info>
  Sie können dieselben Optionen im Argument wie im Anforderungstext für [Create
  Checkout
...
<Warning>
  Die <code>authClient.dodopayments.checkout</code>-Methode ist veraltet. Verwenden Sie
...
<Info>
  Das Webhooks-Plugin verarbeitet Echtzeit-Zahlungsereignisse von Dodo Payments mit
  sicherer Signaturverifizierung. Der Standardendpunkt ist <code>/api/auth/dodopayments/webhooks</code>.
</Info>
...
<AccordionGroup>
  <Accordion title="Plugin-Optionen">
    - <b>client</b> (erforderlich): DodoPayments-Clientinstanz - <b>createCustomerOnSignUp</b> (optional): Kunden automatisch bei der Benutzeranmeldung erstellen - <b>use</b> (erforderlich): Array von Plugins, die aktiviert werden sollen (Checkout, Portal, Nutzung, Webhooks)
  </Accordion>
  <Accordion title="Checkout-Plugin-Optionen">
    - <b>products</b>: Array von Produkten oder asynchrone Funktion, die Produkte zurückgibt - <b>successUrl</b>: URL zur Weiterleitung nach erfolgreicher Zahlung - <b>authenticatedUsersOnly</b>: Benutzerauthentifizierung erforderlich (Standard: false)
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Häufige Probleme">
    - <b>Ungültiger API-Schlüssel</b>: Überprüfen Sie Ihren <code>DODO_PAYMENTS_API_KEY</code> in <code>.env</code>. - <b>Webhook-Signatur stimmt nicht überein</b>: Stellen Sie sicher, dass das Webhook-Geheimnis mit dem im Dodo Payments-Dashboard festgelegten übereinstimmt. - <b>Kunde nicht erstellt</b>: Bestätigen Sie, dass <code>createCustomerOnSignUp</code> auf <code>true</code> gesetzt ist.
  </Accordion>
  <Accordion title="Best Practices">
    - Verwenden Sie Umgebungsvariablen für alle Geheimnisse und Schlüssel. - Testen Sie im <code>test_mode</code>, bevor Sie zu <code>live_mode</code> wechseln. - Protokollieren Sie Webhook-Ereignisse zur Fehlersuche und Prüfung.
  </Accordion>
</AccordionGroup>
