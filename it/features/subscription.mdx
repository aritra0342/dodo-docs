...
Addebita un importo prorato in base al tempo rimanente nel ciclo di fatturazione attuale. Ideale per una fatturazione equa che tiene conto del tempo non utilizzato.
...
<Info>
Le modifiche ai piani attivano addebiti immediati. Gli addebiti non riusciti possono spostare l'abbonamento nello stato `on_hold`. Tieni traccia delle modifiche tramite eventi webhook `subscription.plan_changed`.
</Info>
...
<Info>
Dopo aver aggiornato con successo il metodo di pagamento per un abbonamento in `on_hold`, riceverai eventi webhook `payment.succeeded` seguiti da `subscription.active`.
</Info>
...
Utilizza `POST /subscriptions` per creare abbonamenti programmaticamente da prodotti, con prove e add-on opzionali.
...
Utilizza `PATCH /subscriptions/{id}` per aggiornare le quantità, annullare alla fine del periodo o modificare i metadati.
...
Utilizza `GET /subscriptions` per elencare tutti gli abbonamenti e `GET /subscriptions/{id}` per recuperarne uno.
...
Aggiorna il metodo di pagamento per un abbonamento. Per abbonamenti attivi, questo aggiorna il metodo di pagamento per i rinnovi futuri. Per abbonamenti nello stato `on_hold`, questo riattiva l'abbonamento creando un addebito per le spese rimanenti.
