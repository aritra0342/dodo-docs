...
Configura le variabili d'ambiente nel tuo dashboard Convex (**Impostazioni** → **Variabili d'ambiente**). Puoi accedere al dashboard eseguendo:
...
<Tab title="Configurazione del Portale Clienti">
  <Info>
...
<Tab title="Configurazione del Gestore Webhook">
  <Info>
  Usa questo gestore per ricevere e gestire in modo sicuro gli eventi webhook di Dodo Payments nella tua app Convex. Tutti i gestori webhook ricevono il `ActionCtx` di Convex come primo parametro, consentendoti di utilizzare `ctx.runQuery()` e `ctx.runMutation()` per interagire con il tuo database.
  </Info>
  <CodeGroup>
