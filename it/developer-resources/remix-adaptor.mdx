...
<Info>
Tutti gli esempi presuppongono che tu stia utilizzando il Remix App Router.
</Info>

<Tabs>
<Tab title="Gestore Checkout">
  <Info>
  Usa questo gestore per integrare il checkout di Dodo Payments nella tua app Remix. Supporta flussi di pagamento statici (GET), dinamici (POST) e di sessione (POST).
  </Info>
  <CodeGroup>
...
<Tab title="Gestore Webhook">
  <Info>
  Usa questo gestore per ricevere e gestire in modo sicuro gli eventi webhook di Dodo Payments nella tua app Remix.
  </Info>
  <CodeGroup>
...
Le sessioni di checkout offrono un'esperienza di checkout più sicura e ospitata che gestisce l'intero flusso di pagamento sia per acquisti una tantum che per abbonamenti con pieno controllo sulla personalizzazione.
