...
<CardGroup cols={2}>
<Card title="Inizio Veloce" icon="rocket" href="#integration-workflow">
  Fai partire la tua integrazione dei pagamenti mobili in 4 semplici passaggi
</Card>

<Card title="Esempi di Piattaforma" icon="code" href="#platform-specific-integration">
...
<Steps>
<Step title="Backend: Crea Sessione di Checkout">
<Card title="Documentazione API Sessione di Checkout" icon="book" href="/developer-resources/checkout-session">
Scopri come creare una sessione di checkout nel tuo backend utilizzando Node.js, Python e altro. Vedi esempi completi e riferimenti ai parametri nella documentazione dedicata alla <strong>API delle Sessioni di Checkout</strong>.
</Card>
<Note>
...
<Step title="Mobile: Ottieni URL di Checkout">
...
<Step title="Mobile: Apri Checkout nel Browser">
Apri l'URL di checkout in un browser in-app sicuro per l'elaborazione dei pagamenti.

<Card 
  title="Vedi esempi di integrazione specifici per piattaforma" 
  icon="box" 
...
<Step title="Backend: Gestisci il Completamento del Pagamento">
...
Aggiungi questa dipendenza al tuo `package.json`:
...
<Tip>
Utilizza InAppBrowser per un'esperienza di navigazione nativa. Gestisci i deep link per i callback di pagamento.
</Tip>
...
<Warning>
**Conformità Regionale**: Gli SDK di pagamento in-app hanno restrizioni regionali. Su **iOS**, gli SDK di pagamento nativi sono legali solo nell'**UE** (sotto i termini del DMA). Su **Android**, sono legali nei mercati di User Choice Billing (UCB). Per altre regioni, utilizza l'approccio dei link di checkout sopra per garantire la conformità con l'App Store e il Play Store.
</Warning>

<Card 
  title="Guida all'Integrazione SDK React Native" 
  icon="mobile-screen" 
  href="/developer-resources/react-native-integration"
>
  Guida completa per integrare l'SDK React Native di Dodo con fogli di pagamento nativi, opzioni di personalizzazione complete e gestione diretta dei pagamenti. Include istruzioni di configurazione, implementazione e test.
</Card>

<Info>
Rivedi la [Guida al Bypass delle Commissioni dell'App Store](/features/bypassing-app-store-fees) per comprendere i requisiti di conformità regionale per le integrazioni di pagamento in-app.
</Info>
...
<Tip>
Utilizza WebView per i link di pagamento. Gestisci l'URL di reindirizzamento per rilevare il completamento del pagamento.
</Tip>
