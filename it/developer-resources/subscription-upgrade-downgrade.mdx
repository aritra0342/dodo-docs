...
<Accordion title="Abbonamento in attesa dopo il cambiamento di piano">
**Sintomi**: L'addebito per il cambiamento di piano fallisce e l'abbonamento passa allo stato `on_hold`

**Cosa succede**:
Quando un addebito per il cambiamento di piano fallisce, l'abbonamento viene automaticamente posto nello stato `on_hold`. L'abbonamento non si rinnoverà automaticamente fino a quando il metodo di pagamento non verrà aggiornato.
...
<Step title="Testa diverse modalità di proration">
...
- Rivedi l'<a href="/api-reference/subscriptions/change-plan">API Cambia Piano</a>
