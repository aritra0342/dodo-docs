...
[피그마의 플러그인 빠른 시작 가이드](https://www.figma.com/plugin-docs/plugin-quickstart-guide/)를 사용하여 자신의 피그마 플러그인 프로젝트를 생성하세요. 이렇게 하면 `manifest.json` 파일이 포함된 스캐폴드가 생성됩니다.
...
<Step title="환경 구성">
`API_MODE`를 `src/ui/api.ts`에 설정하세요:
...
- 유효한 라이센스 키를 가진 사용자에게 기능을 노출하기 위해 `Authenticated.tsx`를 업데이트하세요
- 디자인 선호도에 맞게 `LicenseKeyInput.tsx`를 사용자 정의하세요
...
3. [피그마의 플러그인 배포 가이드라인](https://www.figma.com/plugin-docs/publishing-plugins/)을 따르세요
