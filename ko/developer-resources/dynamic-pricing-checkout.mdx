...
<Info>
제품 장바구니에 `amount`를 전달하면 해당 금액이 체크아웃에 사용됩니다. `amount` 필드를 생략하면 고객이 체크아웃 중에 자신의 가격을 선택할 수 있습니다(귀하의 최소/최대 설정에 따라).
</Info>
...
**예**: 최소 가격이 $5.00인 경우 `5.00`(또는 `500` 센트)을 입력합니다.
</Step>
...
<Step title="제품 저장">
**제품 추가**를 클릭하여 저장합니다. 체크아웃 세션에서 사용할 제품 ID(예: `pdt_123abc456def`)를 기록해 두십시오.
</Step>
</Steps>
...
- 대시보드에서 제품에 **원하는 가격 지불**이 활성화되어 있습니다.
- 제품이 **단일 결제**(일회성) 제품이며 구독이 아닙니다.
- 금액이 올바른 형식(최소 통화 단위, 예: USD의 경우 센트)입니다.
</Accordion>

<Accordion title="금액이 최대를 초과하거나 최소 미만입니다">
...
고객이 가격 입력 필드를 보지 못하는 경우 제품 장바구니에서 `amount` 필드를 생략했는지 확인하십시오. `amount`가 제공되면 체크아웃은 해당 정확한 금액을 사용합니다.
</Accordion>
</AccordionGroup>