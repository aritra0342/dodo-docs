...
<Steps>
<Step title="환경 변수 구성">
다음 항목을 <code>.env</code> 파일에 추가하세요:
...
<Step title="서버 측 통합 설정">
<code>src/lib/auth.ts</code>를 생성하거나 업데이트하세요:
...
<Tip>
프로덕션에서는 <code>environment</code>를 <code>live_mode</code>로 설정하세요.
</Tip>
</Step>

<Step title="클라이언트 측 통합 설정">
<code>src/lib/auth-client.ts</code>를 생성하거나 업데이트하세요:
...
<Info>
  새로운 통합을 위해서는 <code>authClient.dodopayments.checkoutSession</code>을 선호하세요.
  레거시 <code>checkout</code> 메서드는 더 이상 사용되지 않으며
  이전 호환성을 위해서만 유지됩니다.
</Info>
...
<Info>
  레거시 <code>checkout</code> 메서드와 달리, <code>checkoutSession</code>
 은 청구 정보를 사전에 요구하지 않으며, 이는 체크아웃 페이지에서 사용자로부터 수집됩니다.
  여전히 <code>billing</code> 키를 인수로 전달하여 이를 오버라이드할 수 있습니다.
</Info>

<Info>
  레거시 <code>checkout</code> 메서드와 유사하게,
  <code>checkoutSession</code>은 고객의 이메일과 이름을
  better-auth 세션에서 가져오지만, <code>customer</code> 객체를 전달하여 이를 오버라이드할 수 있습니다.
</Info>

<Info>
  [체크아웃 세션 생성](https://docs.dodopayments.com/api-reference/checkout-sessions/create) 엔드포인트의 요청 본문으로
  인수에 동일한 옵션을 전달할 수 있습니다.
</Info>

<Note>
  반환 URL은 서버 플러그인에 구성된 <code>successUrl</code>에서 가져옵니다.
  클라이언트 페이로드에 <code>return_url</code>을 포함할 필요가 없습니다.
</Note>

## 레거시 체크아웃 (더 이상 사용되지 않음)

<Warning>
  <code>authClient.dodopayments.checkout</code> 메서드는 더 이상 사용되지 않습니다.
  새로운 구현을 위해서는 <code>checkoutSession</code>을 사용하세요.
</Warning>
...
<Tip>
  한 시간 이상 오래된 타임스탬프나 5분 이상 미래의 타임스탬프는
  사용 수집 시 거부됩니다.
</Tip>
<Tip>
  사용량 미터를 나열할 때 <code>meter_id</code>를 생략하면,
  고객의 구독에 연결된 모든 미터가 반환됩니다.
</Tip>

# 웹훅

<Info>
  웹훅 플러그인은 Dodo Payments의 실시간 결제 이벤트를
  안전한 서명 검증과 함께 처리합니다. 기본 엔드포인트는 <code>/api/auth/dodopayments/webhooks</code>입니다.
</Info>

<Steps>
<Step title="웹훅 비밀 생성 및 설정">
Dodo Payments 대시보드에서 엔드포인트 URL(예: <code>https://&lt;your-domain&gt;/api/auth/dodopayments/webhooks</code>)에 대한 웹훅 비밀을 생성하고 이를 <code>.env</code> 파일에 설정하세요:
...
<AccordionGroup>
  <Accordion title="플러그인 옵션">
    - <b>client</b> (필수): DodoPayments 클라이언트 인스턴스 - <b>createCustomerOnSignUp</b> (선택 사항): 사용자 가입 시 고객 자동 생성 - <b>use</b> (필수): 활성화할 플러그인 배열(체크아웃, 포털, 사용, 웹훅)
  </Accordion>
  <Accordion title="체크아웃 플러그인 옵션">
    - <b>products</b>: 제품 배열 또는 제품을 반환하는 비동기 함수 - <b>successUrl</b>: 결제가 성공한 후 리디렉션할 URL - <b>authenticatedUsersOnly</b>: 사용자 인증 필요(기본값: false)
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="일반적인 문제">
    - <b>유효하지 않은 API 키</b>: <code>.env</code>에서 <code>DODO_PAYMENTS_API_KEY</code>를 다시 확인하세요. - <b>웹훅 서명 불일치</b>: 웹훅 비밀이 Dodo Payments 대시보드에 설정된 것과 일치하는지 확인하세요. - <b>고객이 생성되지 않음</b>: <code>createCustomerOnSignUp</code>가 <code>true</code>로 설정되어 있는지 확인하세요.
  </Accordion>
  <Accordion title="모범 사례">
    - 모든 비밀 및 키에 대해 환경 변수를 사용하세요. - <code>live_mode</code>로 전환하기 전에 <code>test_mode</code>에서 테스트하세요. - 디버깅 및 감사용으로 웹훅 이벤트를 기록하세요.
  </Accordion>
</AccordionGroup>
