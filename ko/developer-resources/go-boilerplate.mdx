...
- **고객 포털** - 셀프 서비스 구독 관리
- **Go 모범 사례** - `cmd`, `internal` 및 `templates`을 사용한 클린 아키텍처
...
<Step title="제품 추가">
Dodo Payments에서 실제 제품 ID로 `internal/lib/products.go`를 업데이트하세요:
...
`internal/lib/products.go`를 편집하여 다음을 수정하세요:
- 제품 ID (Dodo 대시보드에서)
- 가격
- 기능
- 설명
...
`templates/index.html`에서 하드코딩된 고객 데이터를 실제 사용자 데이터로 교체하세요:
...
보일러플레이트는 `internal/api/webhook.go`에서 웹훅 이벤트 처리를 보여줍니다. 지원되는 이벤트는 다음과 같습니다:
...
웹훅 URL을 [Dodo 대시보드](https://app.dodopayments.com/developer/webhooks)에서 ngrok URL로 업데이트하세요. 올바른 웹훅 검증 키로 `.env` 파일을 업데이트하세요.
</Accordion>
