...
<Card title="체크아웃 핸들러" icon="cart-shopping" href="#checkout-route-handler">
    Dodo Payments 체크아웃을 Tanstack 앱에 통합하세요.
</Card>
...
<Info>
모든 예제는 Tanstack App Router를 사용한다고 가정합니다.
</Info>

<Tabs>
<Tab title="체크아웃 핸들러">
  <Info>
  이 핸들러를 사용하여 Dodo Payments 체크아웃을 Tanstack 앱에 통합하세요. 정적 (GET), 동적 (POST), 세션 (POST) 결제 흐름을 지원합니다.
  </Info>
  <CodeGroup>
...
<Tab title="웹훅 핸들러">
  <Info>
  이 핸들러를 사용하여 Tanstack 앱에서 Dodo Payments 웹훅 이벤트를 안전하게 수신하고 처리합니다.
  </Info>
  <CodeGroup>
...
결제 금액 지정 (예: <code>1000</code>는 $10.00).
</ParamField>
...
체크아웃 세션은 일회성 구매 및 구독을 위한 전체 결제 흐름을 처리하는 보다 안전하고 호스팅된 체크아웃 경험을 제공합니다. 완전한 사용자 정의 제어가 가능합니다.
