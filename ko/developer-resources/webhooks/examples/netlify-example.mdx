...
1. [Neon](https://neon.com) 가입
2. 새 프로젝트 생성
3. SQL 편집기 열기
4. [`schema.sql`](#database-schema)의 내용을 복사하여 붙여넣기
5. 쿼리 실행
6. Neon에서 연결 문자열을 가져오기 → 연결 세부정보
...
> **참고:** 배포 후 웹훅 URL을 얻은 후 `DODO_PAYMENTS_WEBHOOK_KEY`을 설정합니다.
...
> **참고:** 이 구현은 최소한의 필드로 세 가지 핵심 구독 이벤트 (`subscription.active`, `subscription.cancelled`, `subscription.renewed`)을 처리하는 방법을 보여줍니다. 필요에 따라 추가 이벤트 유형 및 필드를 지원하도록 쉽게 확장할 수 있습니다.
...
3. **이벤트 기록** - 감사 추적을 위해 `webhook_events` 테이블에 원시 웹훅 저장
4. **이벤트 처리** - Neon에서 고객 및 구독 생성 또는 업데이트
...
| 데이터베이스 연결 오류 | Neon 연결 문자열을 확인하고 풀링된 연결을 사용 |
| 함수 찾을 수 없음 (404) | `netlify init` 및 `npm run deploy`을 다시 실행 |
| 환경 변수 사용 불가 | CLI 또는 대시보드에서 변수를 설정한 후 재배포 |
