...
<Step title="결제 유틸리티 함수 생성">
    ...
    <Note>
    이 함수는 결제 의도를 생성하고 클라이언트 비밀을 반환하는 백엔드 API 엔드포인트가 있다고 가정합니다. 결제 생성을 처리하도록 백엔드가 올바르게 구성되어 있는지 확인하세요. 백엔드 설정 예제는 [API 통합 튜토리얼](/developer-resources/integration-tutorial)을 참조하세요.
    </Note>
</Step>
...
각 색상 카테고리는 UI의 하나 이상의 구성 요소의 색상을 결정합니다. 예를 들어, `primary`는 결제 버튼의 색상을 정의합니다.

| 색상 카테고리 | 용도 |
|----------------|-------|
| `primary` | 결제 버튼 및 선택된 항목의 기본 색상 |
