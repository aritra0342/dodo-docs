...
1. VS Code를 엽니다.
2. 사이드바에서 확장 아이콘을 클릭합니다 (macOS에서는 `Cmd+Shift+X` / Windows/Linux에서는 `Ctrl+Shift+X`를 누릅니다).
...
2. 명령 팔레트를 엽니다 (macOS에서는 `Cmd+Shift+P` / Windows/Linux에서는 `Ctrl+Shift+P`를 누릅니다).
3. "Sentra: Configure"를 입력하고 선택합니다.
4. 프롬프트가 나타나면 Dodo Payments API 키를 입력합니다.
...
2. **설치 확인**: 확장이 설치되었는지 확인하려면 확장으로 이동하여 "Sentra"를 검색합니다.
3. **창 새로 고침**: 명령 팔레트를 사용하여 (`Cmd+Shift+P` / `Ctrl+Shift+P`) "개발자: 창 새로 고침"을 실행합니다.
