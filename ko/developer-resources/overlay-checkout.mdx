...
체크아웃을 열 때 `themeConfig` 객체를 `options` 매개변수에 전달하여 체크아웃 모양을 사용자 정의할 수 있습니다. 테마 구성은 밝은 모드와 어두운 모드를 모두 지원하며, 색상, 테두리, 텍스트, 버튼 및 테두리 반경을 사용자 정의할 수 있습니다.
...
5. **유효한 URL**: create checkout session API에서 유효한 체크아웃 URL을 항상 사용하세요.
...
- 귀하의 프로덕션 도메인 URL (예: `https://example.com`)
