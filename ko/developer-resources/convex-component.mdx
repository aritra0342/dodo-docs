...
`convex.config.ts`를 편집한 후, `npx convex dev`를 한 번 실행하여 필요한 타입을 생성합니다.
</Step>
<Step title="환경 변수 설정">
Convex 대시보드에서 환경 변수를 설정하세요 (**설정** → **환경 변수**). 대시보드에 접근하려면 다음을 실행하세요:
...
- `DODO_PAYMENTS_WEBHOOK_SECRET` - 웹훅 비밀 (웹훅 처리를 위해 필요)
...
<Warning>
이 쿼리를 사용하기 전에 `convex/schema.ts` 파일에 적절한 스키마를 정의하거나 쿼리를 기존 스키마에 맞게 변경하세요.
</Warning>
...
<Tab title="고객 포털 설정">
  <Info>
  이 기능을 사용하여 고객이 Dodo Payments 고객 포털을 통해 구독 및 세부 정보를 관리할 수 있도록 합니다. 고객은 `identify` 기능을 통해 자동으로 식별됩니다.
  </Info>
  <CodeGroup>
...
<Tab title="웹훅 핸들러 설정">
  <Info>
  이 핸들러를 사용하여 Convex 앱에서 Dodo Payments 웹훅 이벤트를 안전하게 수신하고 처리합니다. 모든 웹훅 핸들러는 Convex `ActionCtx`를 첫 번째 매개변수로 수신하여 `ctx.runQuery()` 및 `ctx.runMutation()`를 사용하여 데이터베이스와 상호작용할 수 있습니다.
  </Info>
  <CodeGroup>
...
<Info>
고객은 DodoPayments 설정에서 구성된 `identify` 기능을 사용하여 자동으로 식별됩니다. 이 기능은 고객의 `dodoCustomerId`를 반환해야 합니다.
</Info>
