...
<Step title="웹훅 엔드포인트 생성하기">
    `Add Webhook`를 클릭하여 새 웹훅 엔드포인트를 생성합니다.
...
<Step title="비밀 키 가져오기">
    설정 페이지에서 웹훅 `Secret Key`을 가져옵니다. 이 키를 사용하여 수신된 웹훅의 진위를 검증합니다.
...
<Step title="웹훅 세부정보로 이동하기">
    Dodo Payments 대시보드로 이동하여 `Settings > Webhooks`로 이동합니다.
</Step>
...
1. `webhook-id`, `webhook-timestamp`, 및 정확한 원시 문자열 `payload`를 점으로 구분하여 연결하여 서명된 메시지를 만듭니다 (`.`).
...
<Accordion title="즉시 응답" icon="bolt">
...
<Expandable title="데이터 객체 속성">
  ...
  추가 필드는 이벤트 유형에 따라 다릅니다. 전체 스키마는 이벤트 특정 문서를 참조하세요.
</Expandable>
</ResponseField>
