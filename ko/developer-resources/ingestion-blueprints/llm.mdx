...
<Warning>
개발 중에는 항상 `test_mode`을 사용하여 프로덕션 메트릭에 영향을 주지 않도록 하세요.
</Warning>
</ParamField>
...
<Note>
**Gemini 사고 모드:** 사고/추론 기능이 있는 Gemini 모델을 사용할 때 (예: Gemini 2.5 Pro), SDK는 `thoughtsTokenCount` (추론 토큰)을 `outputTokens`에 자동으로 포함하여 전체 계산 비용을 정확하게 반영합니다.
</Note>
</Accordion>
</AccordionGroup>
...
<Note>
**추론 토큰:** 추론 기능이 있는 모델의 경우, `outputTokens`는 자동으로 완료 토큰과 추론 토큰을 모두 포함합니다.
</Note>
</ParamField>
