...
| `elementId` | `string` | 예 | 체크아웃이 렌더링될 DOM 요소의 `id`. |
...
SDK는 `onEvent` 콜백을 통해 실시간 이벤트를 제공합니다. 인라인 체크아웃의 경우, `checkout.breakdown`는 UI를 동기화하는 데 특히 유용합니다.
...
`manualRedirect`가 활성화되면, 다음 데이터와 함께 `checkout.status` 이벤트를 수신합니다:
...
`manualRedirect`가 활성화되면, 다음 데이터와 함께 `checkout.redirect_requested` 이벤트를 수신합니다:
...
| `finalTotal` | 고객이 실제로 청구되는 금액입니다. 이는 기본 가격 세부정보의 일부가 아닌 추가 외환 조정 또는 현지 결제 수수료를 포함할 수 있습니다. |
| `finalTotalCurrency` | 고객이 실제로 지불하는 통화입니다. 이는 구매력 평가 또는 현지 통화 변환이 활성화된 경우 `currency`와 다를 수 있습니다. |
...
체크아웃을 열 때 `themeConfig` 객체를 `options` 매개변수로 전달하여 체크아웃 모양을 사용자 정의할 수 있습니다. 테마 구성은 밝은 모드와 어두운 모드를 모두 지원하며, 색상, 테두리, 텍스트, 버튼 및 테두리 반경을 사용자 정의할 수 있습니다.
...
<Info>
어두운 모드 구현의 경우, 인라인 체크아웃 프레임과 최적의 시각적 통합을 위해 `#0d0d0d`를 배경색으로 사용하는 것이 좋습니다.
</Info>
...
- `elementId`가 실제로 DOM에 존재하는 `div`의 `id`와 일치하는지 확인하세요.
