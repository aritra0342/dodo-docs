...
<Tip>
  1 時間以上前または 5 分以上先のタイムスタンプは、使用量を取り込む際に拒否されます。
</Tip>
<Tip>
  使用量メーターをリストする際に <code>meter_id</code> を省略すると、その顧客のサブスクリプションに関連付けられたすべてのメーターが返されます。
</Tip>

# Webhook

<Info>
  Webhook プラグインは、Dodo Payments からのリアルタイムの支払いイベントを安全な署名検証で処理します。
  デフォルトのエンドポイントは <code>/api/auth/dodopayments/webhooks</code> です。
</Info>

<Steps>
<Step title="Webhook シークレットの生成と設定">
Dodo Payments ダッシュボードでエンドポイント URL（例：<code>https://&lt;your-domain&gt;/api/auth/dodopayments/webhooks</code>）のための webhook シークレットを生成し、<code>.env</code> ファイルに設定します：
...
<AccordionGroup>
  <Accordion title="プラグインオプション">
    - <b>client</b>（必須）：DodoPayments クライアントインスタンス - <b>createCustomerOnSignUp</b>（オプション）：ユーザーサインアップ時に顧客を自動作成 - <b>use</b>（必須）：有効にするプラグインの配列（チェックアウト、ポータル、使用量、Webhook）
  </Accordion>
  <Accordion title="チェックアウトプラグインオプション">
    - <b>products</b>：製品の配列または製品を返す非同期関数 - <b>successUrl</b>：支払い成功後にリダイレクトする URL - <b>authenticatedUsersOnly</b>：ユーザー認証を要求（デフォルト：false）
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="一般的な問題">
    - <b>無効な API キー</b>：<code>DODO_PAYMENTS_API_KEY</code> を <code>.env</code> で再確認してください。 - <b>Webhook 署名の不一致</b>：Webhook シークレットが Dodo Payments ダッシュボードで設定されたものと一致していることを確認してください。 - <b>顧客が作成されない</b>：<code>createCustomerOnSignUp</code> が <code>true</code> に設定されていることを確認してください。
  </Accordion>
  <Accordion title="ベストプラクティス">
    - すべてのシークレットとキーに環境変数を使用してください。 - <code>live_mode</code> に切り替える前に <code>test_mode</code> でテストしてください。 - デバッグと監査のために Webhook イベントをログに記録してください。
  </Accordion>
</AccordionGroup>
