...
<Info>
このボイラープレートは、クリーンアーキテクチャパターン（`cmd`、`internal`、`templates`）、HTMLテンプレート、およびシームレスなAPI統合のための`dodopayments-go` SDKを使用して、Go 1.21+で動作します。
</Info>
...
<Step title="製品を追加">
`internal/lib/products.go`をDodo Paymentsからの実際の製品IDで更新します：
...
`internal/lib/products.go`を編集して、以下を変更します：
- 製品ID（Dodoダッシュボードから）
- 価格
- 特徴
- 説明
...
ボイラープレートは、`internal/api/webhook.go`でWebhookイベントの処理を示しています。サポートされているイベントは次のとおりです：
...
<Accordion title="チェックアウトセッションの作成に失敗しました">
**一般的な原因：**
...
Webhook URLを[ダッシュボード](https://app.dodopayments.com/developer/webhooks)でngrokのURLに更新します。正しいWebhook検証キーで`.env`ファイルを更新することを忘れないでください。
</Accordion>
