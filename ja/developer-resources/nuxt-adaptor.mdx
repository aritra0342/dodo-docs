...
<CardGroup cols={2}>
<Card title="チェックアウト API ルート" icon="cart-shopping" href="#checkout-api-route">
    Dodo Payments のチェックアウトをサーバールートを使用して Nuxt アプリに統合します。
</Card>
<Card title="顧客ポータル API ルート" icon="user" href="#customer-portal-api-route">
    顧客が Nuxt サーバールートを介してサブスクリプションや詳細を管理できるようにします。
</Card>
<Card title="ウェブフック API ルート" icon="bell" href="#webhook-api-route">
    Nuxt で Dodo Payments のウェブフックイベントを安全に受信および処理します。
</Card>
</CardGroup>
...
<Info>
このガイドでは、公式の Nuxt モジュールを使用して Dodo Payments を Nuxt アプリケーションに統合する方法を説明します。チェックアウト、顧客ポータル、およびウェブフック API ルートの設定方法と、環境変数を安全に管理する方法を学びます。
</Info>

## インストール

<Steps>
<Step title="Nuxt モジュールをインストールする">
...
<Step title="nuxt.config.ts にモジュールを登録する">
...
<Info>
Nuxt におけるすべての Dodo Payments 統合は、<code>server/routes/api/</code> ディレクトリ内のサーバールートを介して処理されます。
</Info>

<Tabs>
<Tab title="チェックアウト API ルート">
<Info>
このハンドラーを使用して、Dodo Payments のチェックアウトを Nuxt アプリに統合します。静的 (GET)、動的 (POST)、およびセッション (POST) の支払いフローをサポートします。
</Info>
<CodeGroup>
...
<Warning>
<code>productId</code> が欠落しているか無効な場合、ハンドラーは 400 のレスポンスを返します。
</Warning>
...
<Tab title="顧客ポータル API ルート">
<Info>
顧客がポータルにアクセスできるようにする GET ルートを作成します。<code>customer_id</code> をクエリパラメータとして受け入れます。
</Info>
<CodeGroup>
...
<Warning>
<code>customer_id</code> が欠落している場合、400 を返します。
</Warning>
...
<Tab title="ウェブフック API ルート">
<Info>
Dodo Payments からのウェブフックイベントを安全に受信および処理するための POST ルートを作成します。
</Info>
<CodeGroup>
...
<Info>
Dodo Payments は、ウェブサイトに支払いを統合するための 3 種類の支払いフローをサポートしており、このアダプタはすべてのタイプの支払いフローをサポートしています。
</Info>
...
支払い金額を指定します (例: <code>1000</code> は $10.00 を意味します)。
</ParamField>
...
<Warning>
<code>productId</code> が欠落している場合、ハンドラーは 400 のレスポンスを返します。無効なクエリパラメータも 400 のレスポンスを返します。
</Warning>
...
チェックアウトセッションは、完全な支払いフローを処理するより安全でホストされたチェックアウト体験を提供し、一回限りの購入とサブスクリプションの両方に対してフルカスタマイズ制御を行います。
