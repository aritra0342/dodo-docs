...
**フォーマット**: 国コードを含めてください。例: `"+1234567890"`（米国の番号）
</ParamField>
</Expandable>
</Tab>
</Tabs>
...
**フォーマット**: 通貨の最小単位で表されます（例: セントでUSD）。例えば、$1.00を請求するには、`100`を渡します。
</ParamField>
...
<Note>
`redirect_immediately`が有効になっている場合、顧客は支払い完了後に即座に`return_url`にリダイレクトされ、デフォルトの成功ページを完全にスキップします。
</Note>
