...
<Tip>
`interval`オプションは、自動同期のために`.start()`を使用する場合に必要です。プロセスが停止されるまで、指定された間隔で同期が継続的に実行されます。
</Tip>
...
<Warning>
**MongoDB**: `dodopayments_sync`という名前のデータベースが自動的にデータベースサーバー上に作成されます。すべての同期データはそこに保存されます。このデータベース名は現在固定されており、変更することはできません。
