...
<Tip>
Webhookを非同期に処理し、`200`ステータスコードで受信を即座に確認し、その後実際の処理をバックグラウンドで行います。
</Tip>
...
各Webhookリクエストには、WebhookペイロードとタイムスタンプのHMAC SHA256署名を含む`webhook-signature`ヘッダーが含まれています。これはあなたの秘密鍵で署名されています。
...
<Tip>
  Dodo Paymentsクライアントを初期化する際に、`DODO_PAYMENTS_WEBHOOK_KEY`を介してWebhook秘密を提供してください。
</Tip>
...
3. 計算された署名を`webhook-signature`ヘッダーと比較します。一致すれば、Webhookは本物です。
...
<Accordion title="即座に応答する" icon="bolt">
...
<Accordion title="重複イベントを処理する" icon="rotate">
...
<Expandable title="データオブジェクトのプロパティ">
  ...
  追加のフィールドはイベントタイプによって異なります。完全なスキーマについては、イベント特有のドキュメントを参照してください。
</Expandable>
</ResponseField>
