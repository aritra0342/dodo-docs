...
<Card title="チェックアウトハンドラー" icon="cart-shopping" href="#checkout-route-handler">
    Dodo Paymentsのチェックアウトを静的、動的、セッションフローと統合します。
</Card>
...
<Info>
すべての例は、Next.js App Routerを使用していることを前提としています。
</Info>

<Tabs>
<Tab title="チェックアウトハンドラー">
  <Info>
  このハンドラーを使用して、Dodo PaymentsのチェックアウトをNext.jsアプリに統合します。静的（GET）、動的（POST）、およびチェックアウトセッション（POST）の支払いフローをサポートします。
  </Info>
  <CodeGroup>
...
チェックアウトセッションは、ワンタイム購入とサブスクリプションの完全な支払いフローを処理する、安全でホストされたチェックアウト体験を提供します。
