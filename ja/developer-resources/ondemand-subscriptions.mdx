...
このガイドを使用して:
- オンデマンドサブスクリプションを作成する（オプションの初期価格での権限付与）
...
<Tip>
顧客にホストされたチェックアウトを介して権限を承認させたい場合は、`payment_link: true`を設定し、`return_url`を提供してください。
</Tip>
...
<Accordion title="リクエストボディパラメータ">
  ...
  <ParamField body="payment_link" type="boolean" default="false">
    trueの場合、権限付与とオプションの初期支払いのためのホストされたチェックアウトリンクを作成します。
  </ParamField>
  ...
  <ParamField body="on_demand.mandate_only" type="boolean" required>
    trueの場合、作成中に顧客に請求せずに支払い方法を認証します。
  </ParamField>
  ...
  <ParamField body="on_demand.product_currency" type="string">
    初期請求のためのオプションの通貨オーバーライド。製品の通貨がデフォルトです。
  </ParamField>
...
<Check>
`payment_link: true`を設定し、顧客を`payment_link`にリダイレクトして権限付与を完了させます。
</Check>
...
当社の不正検出システムは、攻撃的な再試行パターンをブロックする場合があります（これを潜在的なカードテストとしてフラグ付けすることがあります）。安全な再試行ポリシーに従ってください。
...
最終ステップ: まだ未払いの場合は、サブスクリプションを未払いとしてマークするか、ポリシーに基づいてキャンセルします。顧客に支払い方法を更新するためのウィンドウ中に通知します。

### バースト再試行を避け、承認時間に合わせる

- 再試行を元の承認タイムスタンプに固定して、ポートフォリオ全体で「バースト」動作を避けます。
- 例: 顧客が今日の午後1時10分にトライアルまたは権限を開始した場合、バックオフに従って次の再試行を午後1時10分にスケジュールします（例: +3日→午後1時10分、+7日→午後1時10分）。
- あるいは、最後の成功した支払い時間を保存している場合は、`T`で次の試行をスケジュールして、時間帯の整合性を保ちます。
...
- 正確なタイムスタンプを持続するスケジューラー/キューを使用し、次の試行を正確な時間帯オフセットで計算します（例: `T + 3 days`を同じHH:MMで）。
- 最後の成功した支払いタイムスタンプ`T`を維持し、次の試行を計算します。複数のサブスクリプションを同時にバンチしないでください。
- 最後の拒否理由を常に評価します。上記のスキップリストにあるハード拒否の再試行を停止します。
...
- **subscription.active**: 権限が付与され、サブスクリプションがアクティブ化されました
- **subscription.failed**: 作成に失敗しました（例: 権限の失敗）
...
テストAPIキーを使用して、`payment_link: true`でサブスクリプションを作成し、リンクを開いて権限を完了します。
</Step>
<Step title="請求をトリガー">
