...
オンデマンドサブスクリプションを使用すると、顧客の支払い方法を一度認証し、その後必要に応じて変動額を請求できます。固定スケジュールではなく、必要なときに請求します。

<Info>
オンデマンドサブスクリプションはすべてのビジネス向けに利用可能になりました。この機能により、使用量ベースおよびメーターサービスの柔軟な請求管理が可能になります。
</Info>
...
<Tip>
顧客にホストされたチェックアウトを介して委任を承認させたい場合は、`payment_link: true`を設定し、`return_url`を提供してください。
</Tip>

## オンデマンドの仕組み

1. `on_demand`オブジェクトを使用してサブスクリプションを作成し、支払い方法を承認し、オプションで初期料金を収集します。
...
<Accordion title="リクエストボディパラメータ">
  ...
  <ParamField body="payment_link" type="boolean" default="false">
    trueの場合、委任承認とオプションの初期支払いのためのホストされたチェックアウトリンクを作成します。
  </ParamField>
  ...
  <ParamField body="on_demand.product_currency" type="string">
    初期請求のためのオプションの通貨上書き。デフォルトは製品通貨です。
  </ParamField>
  ...
  <ParamField body="on_demand.adaptive_currency_fees_inclusive" type="boolean">
    trueの場合、<code>product_price</code>に適応通貨手数料を含めます。falseの場合、手数料は上乗せされます。適応価格設定が無効になっている場合は無視されます。
  </ParamField>
</Accordion>
</AccordionGroup>
...
<Check>
`payment_link: true`を設定し、顧客を`payment_link`にリダイレクトして委任承認を完了させます。
</Check>
...
<Warning>
オンデマンドでないサブスクリプションに請求すると失敗する可能性があります。請求する前に、サブスクリプションの詳細に`on_demand: true`が含まれていることを確認してください。
</Warning>

## 支払いの再試行

当社の不正検出システムは、攻撃的な再試行パターンをブロックする場合があります（これらを潜在的なカードテストとしてフラグ付けすることがあります）。安全な再試行ポリシーに従ってください。

<Warning>
バースト再試行パターンは、不正または疑わしいカードテストとして当社のリスクシステムおよびプロセッサーによってフラグ付けされる可能性があります。クラスター化された再試行を避け、以下のバックオフスケジュールおよび時間調整ガイダンスに従ってください。
</Warning>

### 安全な再試行ポリシーの原則

- **バックオフメカニズム**: 再試行の間に指数バックオフを使用します。
- **再試行制限**: 合計再試行を制限します（最大3〜4回の試行）。
...
- あるいは、最後の成功した支払い時間を保存している場合は、`T`、次の試行を`T + X days`でスケジュールして、時間帯の整合性を保ちます。
...
<Tip>
ソフト/一時的な問題（例: `insufficient_funds`、`issuer_unavailable`、`processing_error`、ネットワークタイムアウト）でのみ再試行してください。同じ拒否が繰り返される場合は、さらなる再試行を一時停止してください。
</Tip>
...
- 顧客およびアカウントごとに同時再試行を制限して、偶発的な急増を防ぎます。
...
<Tip>
オンデマンドフローの場合、使用量ベースの請求を調整するために`payment.succeeded`および`payment.failed`に焦点を当ててください。
</Tip>
...
- **通貨エラー**: `product_currency`を上書きする場合は、アカウントおよび顧客に対してサポートされていることを確認してください。
