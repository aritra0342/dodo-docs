...
<Card title="チェックアウトハンドラー" icon="cart-shopping" href="#checkout-route-handler">
    Dodo Payments のチェックアウトを Express アプリに統合します。
</Card>
...
<Warning>
<code>.env</code> ファイルや秘密情報をバージョン管理にコミットしないでください。
</Warning>
</Step>
</Steps>
...
<Tab title="チェックアウトハンドラー">
  <Info>
  このハンドラーを使用して、Dodo Payments のチェックアウトを Express アプリに統合します。静的 (GET)、動的 (POST)、およびセッション (POST) の支払いフローをサポートします。
  </Info>
  <CodeGroup>
...
<Tab title="カスタマーポータルハンドラー">
  <Info>
  このハンドラーを使用して、顧客が Dodo Payments のカスタマーポータルを通じてサブスクリプションや詳細を管理できるようにします。
  </Info>
  <CodeGroup>
...
<Info>
Dodo Payments は、ウェブサイトに支払いを統合するための 3 種類の支払いフローをサポートしており、このアダプタはすべてのタイプの支払いフローをサポートします。
</Info>
...
<Warning>
<code>productId</code> が欠けている場合、ハンドラーは 400 のレスポンスを返します。無効なクエリパラメータも 400 のレスポンスになります。
</Warning>
