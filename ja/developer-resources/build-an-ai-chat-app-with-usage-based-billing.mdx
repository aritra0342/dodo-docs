...
<Info>
「合計」を使用しているのは、複数のチャットメッセージで消費されたすべてのトークンを合計したいからです。SDKは各イベントで`totalTokens`を自動的に送信します。
</Info>
</Step>
...
さらに、**固定価格**を`0`に設定して、顧客が使用に基づいてのみ請求され、基本料金が発生しないようにします。
...
1. `.env`ファイルにステップ2からのすべてのAPIキーが含まれていることを確認します
2. 開発サーバーを起動します：
...
<Note>
`server.ts`の`CUSTOMER_ID`を、ステップ4からの実際のテスト顧客IDで更新してください。
</Note>
</Step>
