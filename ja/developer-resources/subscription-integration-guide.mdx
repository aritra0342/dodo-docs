...
2. `payment.succeeded` - 初回支払いを確認します：
    ...
    - トライアル日：終了時
3. `subscription.renewed` - 支払いの控除と次のサイクルの更新を示します。（基本的に、サブスクリプション製品の支払いが控除されるたびに、`subscription.renewed`のWebhookが`payment.succeeded`と共に送信されます）
...
サブスクリプションが`on_hold`状態に入ると、再アクティブ化するために支払い方法を更新する必要があります。このセクションでは、サブスクリプションが保留中になる理由とその処理方法について説明します。
...
`subscription.on_hold` Webhookを受信したときは、アプリケーションの状態を更新し、顧客に通知します：
