...
<Note>
Node.js 18以上が必要です。`mcp-remote`パッケージがリモートMCPサーバーへの接続を処理します。
</Note>
...
4. **Node.jsのバージョンを確認**: Node.js 18+が必要
...
<Tip>
常に最新バージョンを取得するには`@latest`を使用するか、安定性のために特定のバージョンにピン留めします（`@1.0.0`）。
</Tip>
...
<Tab title="Claude Desktop">
Claude DesktopでDodo Payments MCPサーバーを設定して、支払いデータへの会話アクセスを有効にします。
<Steps>
...
クリック後、VS Codeの`mcp.json`で環境変数を設定します。**コマンドパレット** > **MCP: ユーザー設定を開く**を通じて行います。
...
<Step title="ウィンドウを再読み込み">
    VS Codeを再読み込みします（`Cmd/Ctrl + Shift + P` > "ウィンドウを再読み込み"）して、MCPサーバーを有効にします。
</Step>
</Steps>
</Tab>

<Tab title="Cline (VS Code)">
Cline (VS Code)でDodo Payments MCPサーバーを設定して、支払いデータへの会話アクセスを有効にします。
<Steps>
...
<Step title="VS Codeを再読み込み">
    VS Codeウィンドウを再読み込みします（`Cmd/Ctrl + Shift + P` > "ウィンドウを再読み込み"）して、MCPサーバー接続を有効にします。
</Step>
</Steps>
</Tab>
...
<Tip>
開発中は`--operation=read`を使用して、データの偶発的な変更を防ぎます。
</Tip>
...
1. **`list_api_endpoints`** - オプションのフィルタリングを使用して利用可能なメソッドを発見
...
`DODO_PAYMENTS_ENVIRONMENT`が正しく設定されていることを確認します（本番用には`live_mode`）。
