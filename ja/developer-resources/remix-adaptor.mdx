...
<Card title="チェックアウトハンドラー" icon="cart-shopping" href="#checkout-route-handler">
    Dodo PaymentsのチェックアウトをあなたのRemixアプリに統合します。
</Card>
...
<Tab title="チェックアウトハンドラー">
  <Info>
  このハンドラーを使用して、Dodo PaymentsのチェックアウトをあなたのRemixアプリに統合します。静的（GET）、動的（POST）、およびセッション（POST）支払いフローをサポートします。
  </Info>
  <CodeGroup>
...
<Tab title="ウェブフックハンドラー">
  <Info>
  このハンドラーを使用して、Dodo PaymentsのウェブフックイベントをあなたのRemixアプリで安全に受信して処理します。
  </Info>
  <CodeGroup>
