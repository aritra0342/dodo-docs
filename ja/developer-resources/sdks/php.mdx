...
<Note>
Dodo Payments PHP APIライブラリは現在**ベータ**版です。ぜひお試しください！提案、バグレポート、機能リクエストは、[issueを提出する](https://github.com/dodopayments/dodopayments-php/issues)ことでお知らせください。
</Note>
...
SDKは、オプションの引数を指定するために名前付きパラメータを使用します。静的な`with`コンストラクタを使用して値オブジェクトを初期化できます：
...
ライブラリがAPIに接続できない場合や、非成功ステータスコード（4xxまたは5xx）を受け取った場合、`APIException`のサブクラスがスローされます：
...
<Note>
同じ名前の`extra*`パラメータは、ドキュメントに記載されたパラメータを上書きします。
</Note>
