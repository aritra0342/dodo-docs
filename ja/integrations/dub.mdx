...
1. **チェックアウトセッションを作成する際に、`dub_id`クッキーからDubのクリックIDを取得する**
2. **顧客の外部IDとともに、支払いメタデータにクリックIDを保存する**
3. **支払いが成功したときに、DubのTrack APIを使用して販売データを送信する**
...
1. ワークスペースを持つ[Dubアカウント](https://dub.co)
2. リンクの転換追跡が有効になっている
...
<Steps>
<Step title="Dubでの転換追跡を有効にする">
  Dubダッシュボードで、販売を追跡したいリンクの転換追跡を有効にします。これにより、顧客が購入を完了したときにDubが販売イベントを記録できるようになります。

  <Info>
  [Dubのドキュメント](https://dub.co/docs/conversions/quickstart)で転換追跡を有効にする方法について詳しく学びましょう。
  </Info>
</Step>
...
<Step title="DubのAPIキーを取得する">
  ...
  <Warning>
  APIキーを安全に保管し、クライアント側のコードに公開しないでください。
  </Warning>
</Step>
...
<Step title="Webhookを介して販売データを送信する">
...
- **オーガニックトラフィックを適切に処理する**: クリックIDがない場合は、不要なAPI呼び出しを避けるために`webhook.cancel = true`を設定してください
...
- `dub_id`クッキーがあなたのドメインに正しく設定されていることを確認してください
...
- `payment.succeeded`イベントでのみ追跡していることを確認してください。`payment.processing`ではありません
...
<CardGroup cols={2}>
<Card title="Dubの転換ドキュメント" icon="book" href="https://dub.co/docs/conversions/quickstart">
  Dubの転換追跡と分析機能について詳しく学びましょう。
</Card>
...
<Card title="Dubダッシュボード" icon="chart-line" href="https://app.dub.co">
  Dubダッシュボードにアクセスして、転換分析と帰属データを表示します。
</Card>
...
<Info>
助けが必要ですか？統合に関するサポートは、[support@dodopayments.com](mailto:support@dodopayments.com)までお問い合わせください。
</Info>
