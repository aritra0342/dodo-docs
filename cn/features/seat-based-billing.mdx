...
| 模式 | 添加座位 | 移除座位 |
|------|-------------|----------------|
| `prorated_immediately` | 按周期剩余天数收费 | 对未使用的天数进行信用 |
| `difference_immediately` | 收取全额座位费用 | 信用适用于未来续订 |
...
| `subscription.plan_changed` | 添加/移除座位 | 更新您应用中的座位数量 |
| `subscription.renewed` | 订阅续订 | 确认座位数量未更改 |
