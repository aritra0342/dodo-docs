...
产品可以是一次性项目或基于订阅的服务。要创建新产品，请向 `POST` 端点发送 `/products` 请求，提供名称、描述、价格、货币以及是否为重复产品等详细信息。

对于重复产品，设置 `recurring: true` 并指定 `billing_cycle`（`daily`，`weekly`，`monthly`，`yearly`）。
...
要修改现有产品，请向 `PATCH` 端点发送 `/products/{product_id}` 请求。您可以更新名称、价格和描述等属性，同时保持其他详细信息不变。

确保 `product_id` 端点中的内容与现有产品匹配。
