...
| `type` | string | 是 | 事件的类型。请参见 [事件类型](#event-types) |
| `data` | object | 是 | 主要数据有效负载。请参见 [数据对象](#data-object) |
...
要创建按需订阅，请使用 [POST /subscriptions](/api-reference/subscriptions/post-subscriptions) API 端点，并在请求体中包含 `on_demand` 字段。这允许您在没有立即收费的情况下授权支付方式，或设置自定义初始价格。
