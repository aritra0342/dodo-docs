---
title: SveleteKit 适配器
description: 了解如何使用我们的 SveleteKit 适配器将 Dodo Payments 集成到您的 SveleteKit
  应用路由器项目中。涵盖结账、客户门户、Webhook 和安全环境设置。
icon: code-merge
---

<CardGroup cols={2}>
<Card title="结账处理器" icon="cart-shopping" href="#checkout-route-handler">
    将 Dodo Payments 结账集成到您的 SveleteKit 应用中。
</Card>
...
<Info>
所有示例假设您正在使用 SveleteKit 应用路由器。
</Info>

<Tabs>
<Tab title="结账处理器">
  <Info>
  使用此处理器将 Dodo Payments 结账集成到您的 SveleteKit 应用中。支持静态（GET）、动态（POST）和会话（POST）支付流程。
  </Info>
  <CodeGroup>
...
<Tab title="Webhook 处理器">
  <Info>
  使用此处理器在您的 SveleteKit 应用中安全地接收和处理 Dodo Payments Webhook 事件。
  </Info>
  <CodeGroup>
...
客户门户路由处理器使您能够将 Dodo Payments 客户门户无缝集成到您的 SveleteKit 应用中。
