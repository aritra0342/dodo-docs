...
<Tab title="Claude Desktop">
在 Claude Desktop 中设置 Dodo Payments MCP 服务器，以便能够对您的支付数据进行对话访问。
<Steps>
<Step title="找到配置文件">
...
在 Cline (VS Code) 中设置 Dodo Payments MCP 服务器，以便能够对您的支付数据进行对话访问。
<Steps>
...
可用能力：
- `top-level-unions` - 对顶级联合类型的支持
- `valid-json` - 正确的 JSON 字符串参数解析
- `refs` - 对模式中的 $ref 指针的支持
- `unions` - 对联合类型（anyOf）的支持
- `formats` - 对格式验证的支持
