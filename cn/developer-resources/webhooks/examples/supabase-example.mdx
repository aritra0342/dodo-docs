...
1. 转到您的 [Supabase 仪表板](https://supabase.com/dashboard)
2. 打开 SQL 编辑器
3. 创建一个新查询
...
该函数已经在 `functions/webhook/index.ts` 中设置 - 只需部署它：
...
1. 转到 [DodoPayments 仪表板](https://app.dodopayments.com) → 开发者 → Webhooks
2. 创建一个新的 Webhook 端点
...
✅ **事件日志** - 在 `webhook_events` 表中完整的审计跟踪  
✅ **错误处理** - 记录并可重试
...
<Note>
`--no-verify-jwt` 标志是必需的，因为 Webhook 不包含 JWT 令牌。安全性由 Webhook 签名验证提供。
</Note>
