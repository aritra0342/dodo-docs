...
<Tip>
将 <code>environment</code> 设置为 <code>live_mode</code> 以用于生产环境。
</Tip>
</Step>
...
<Info>
  对于新的集成，首选使用 <code>authClient.dodopayments.checkoutSession</code>。
  传统的 <code>checkout</code> 方法已被弃用，仅保留以支持向后兼容。
</Info>
...
<Info>
  与传统的 <code>checkout</code> 方法不同，<code>checkoutSession</code>
  不需要提前提供账单信息，因为它将在结账页面从用户那里获取。
  您仍然可以通过在参数中传递 <code>billing</code> 键来覆盖它。
</Info>

<Info>
  与传统的 <code>checkout</code> 方法类似，<code>checkoutSession</code>
  从客户的 better-auth 会话中获取客户的电子邮件和姓名，
  但是，您可以通过传递包含电子邮件和姓名的 <code>customer</code> 对象来覆盖它。
</Info>

<Info>
  您可以在参数中传递与 [创建结账会话](https://docs.dodopayments.com/api-reference/checkout-sessions/create)
  端点请求体相同的选项。
</Info>

<Note>
  返回 URL 来自服务器插件中配置的 <code>successUrl</code>。
...
<Warning>
  <code>authClient.dodopayments.checkout</code> 方法已弃用。
  对于新的实现，请使用 <code>checkoutSession</code>。
</Warning>
...
<Tip>
  在摄取使用时，时间戳早于一小时或超过五分钟的未来时间将被拒绝。
</Tip>
<Tip>
  如果在列出使用计量时省略 <code>meter_id</code>，将返回与客户订阅相关的所有计量。
</Tip>

# Webhooks

<Info>
  Webhooks 插件处理来自 Dodo Payments 的实时支付事件，并进行安全的签名验证。
  默认端点是 <code>/api/auth/dodopayments/webhooks</code>。
</Info>

<Steps>
<Step title="生成并设置 webhook 密钥">
为您的端点 URL（例如 <code>https://&lt;your-domain&gt;/api/auth/dodopayments/webhooks</code>）在 Dodo Payments 仪表板中生成 webhook 密钥，并将其设置在您的 <code>.env</code> 文件中：
...
<AccordionGroup>
  <Accordion title="插件选项">
    - <b>client</b>（必需）：DodoPayments 客户端实例 - <b>createCustomerOnSignUp</b>（可选）：在用户注册时自动创建客户 - <b>use</b>（必需）：要启用的插件数组（结账、门户、使用、webhooks）
  </Accordion>
  <Accordion title="结账插件选项">
    - <b>products</b>：产品数组或返回产品的异步函数 - <b>successUrl</b>：成功支付后重定向的 URL - <b>authenticatedUsersOnly</b>：要求用户身份验证（默认：false）
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="常见问题">
    - <b>无效的 API 密钥</b>：仔细检查您的 <code>DODO_PAYMENTS_API_KEY</code> 在 <code>.env</code> 中。 - <b>Webhook 签名不匹配</b>：确保 webhook 密钥与 Dodo Payments 仪表板中设置的密钥匹配。 - <b>客户未创建</b>：确认 <code>createCustomerOnSignUp</code> 设置为 <code>true</code>。
  </Accordion>
  <Accordion title="最佳实践">
    - 对所有机密和密钥使用环境变量。 - 在切换到 <code>live_mode</code> 之前，在 <code>test_mode</code> 中进行测试。 - 记录 webhook 事件以进行调试和审计。
  </Accordion>
</AccordionGroup>
