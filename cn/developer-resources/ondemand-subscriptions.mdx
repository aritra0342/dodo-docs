...
1. 您使用 `on_demand` 对象创建订阅以授权支付方式，并可选地收取初始费用。
...
<Accordion title="请求主体参数">
  ...
  <ParamField body="on_demand.product_price" type="integer">
    初始收费金额（以最小货币单位计）。如果指定，则此值将覆盖在产品创建期间设置的产品原始价格。如果省略，则使用产品存储的价格。示例：要收费 $1.00，请传递 <code>100</code>。
  </ParamField>
...
实现 Webhook 处理以跟踪客户旅程。请参阅 [实现 Webhook](/developer-resources/integration-guide#implementing-webhooks)。
...
- **货币错误**：如果您覆盖 `product_currency`，请确认它对您的帐户和客户是支持的。
