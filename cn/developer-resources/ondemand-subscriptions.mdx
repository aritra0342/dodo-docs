...
<Accordion title="请求主体参数">
  ...
  <ParamField body="on_demand.product_price" type="integer">
    初始收费金额（以最小货币单位计）。如果指定，则此值将覆盖在产品创建期间设置的产品原始价格。如果省略，则使用产品存储的价格。示例：要收取 $1.00，请传递 <code>100</code>。
  </ParamField>
...
<AccordionGroup>
  <Accordion title="收费请求主体参数">
    <ParamField body="product_price" type="integer" required>
      收费金额（以最小货币单位计）。示例：要收取 $25.00，请传递 <code>2500</code>。
    </ParamField>
...
<Tip>
仅在软/临时问题（例如， `insufficient_funds`, `issuer_unavailable`, `processing_error`, 网络超时）时重试。如果相同的拒绝重复，请暂停进一步的重试。
</Tip>
